    ...
    try {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        acts.push({
          ...
          sch_pct, perf_pct, float: float_v, floatCat,
        });
      }

      setHotelTasks(acts);
      setParseMsg(`✓ ${acts.length} activities parsed from "${sheetName}"`);
      setParsing(false);
                ...
                return (
                  <div key={zone}>
                    ...
                    {levels.map((lvl,li)=>{
                      ...
                      const pct  = lA.filter(a=>a.sch_pct>0).length;
                      const avgPct = lA.length>0?(lA.reduce((s,a)=>s+a.sch_pct,0)/lA.length).toFixed(0):0;
