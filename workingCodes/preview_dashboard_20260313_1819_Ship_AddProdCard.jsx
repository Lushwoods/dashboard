      ...
      <div style={{background:C.nav,borderBottom:`1px solid ${C.border}`,padding:"12px 20px 0 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",paddingTop:7,paddingBottom:10}}>
          ...
          <div style={{display:"flex",gap:4}}>
            {[["gantt","📊 Gantt"],["analysis","📈 Productivity"],["cards","🃏 Prod Cards"],["resources","📋 Resources"],["health","🏥 Schedule Health"],["hedging","⚖️ Hedging"]].map(([v,l])=>(
...
const PC_QUARTERS = [
  { key:'25-Q3', label:'25-Q3', sub:'Jul–Sep 2025', mRange:'M11–M13', s:'2025-07-01', e:'2025-09-30' },
  { key:'25-Q4', label:'25-Q4', sub:'Oct–Dec 2025', mRange:'M14–M16', s:'2025-10-01', e:'2025-12-31' },
  { key:'26-Q1', label:'26-Q1', sub:'Jan–Mar 2026', mRange:'M17–M19', s:'2026-01-01', e:'2026-03-31' },
  { key:'26-Q2', label:'26-Q2', sub:'Apr–Jun 2026', mRange:'M20–M22', s:'2026-04-01', e:'2026-06-30' },
  { key:'26-Q3', label:'26-Q3', sub:'Jul–Sep 2026', mRange:'M23–M25', s:'2026-07-01', e:'2026-09-30' },
  { key:'26-Q4', label:'26-Q4', sub:'Oct–Dec 2026', mRange:'M26–M28', s:'2026-10-01', e:'2026-12-31' },
  { key:'27-Q1', label:'27-Q1', sub:'Jan–Mar 2027', mRange:'M29–M31', s:'2027-01-01', e:'2027-03-31' },
];
...
function QSummaryCard({ qDef, qIdx, tasks, areaAllTaskZones, zoneDef, areaLabel, isSelected, col, C, onClick }) {
  ...
  const wtStats  = wtKeys.map(wk => {
    ...
    const ccVol  = wTasks.reduce((s,t)=>s+(t.volume||0),0);
    const sitVol = wTasks.filter(t=>t.actual_s).reduce((s,t)=>s+(t.site_qty||t.volume||0),0);
    const totalCCVol = allWTasks.reduce((s,t)=>s+(t.volume||0),0); // total across all quarters
    ...
    >
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:3 }}>
          <span style={{ fontSize:8, fontWeight:800, background:sc.color+'22', color:sc.color,
            padding:'2px 7px', borderRadius:20, border:`1px solid ${sc.color}44`, whiteSpace:'nowrap' }}>
            {isCurr?'● ':''}{sc.label}
          </span>
          <span style={{ fontSize:8, color:C.sub }}>{withSite}/{total} tasks w/ site</span>
        </div>
      </div>
...
function ProductivityCardView({ tasks, col, areaAllTaskZones, zoneDef, areaLabel, C }) {
  const today    = new Date().toISOString().slice(0,10);
  const defaultQ = PC_QUARTERS.find(q => today >= q.s && today <= q.e)?.key || '26-Q1';
...
function ResourceTrackerView({ tasks, col, areaAllTaskZones, zoneDef, C }) {
  ...
  const [benchmarks, setBenchmarks] = useS({ ...DEFAULT_BENCHMARKS });
  const [editing, setEditing] = useS(null); // { zone, workType, field }

  // Resource register: { [zone]: { [workType]: { planMan, actMan, planEquip, actEquip } } }
  const [register, setRegister] = useS(() => {
    const r = {};
    RES_ZONES.forEach(z => {
      r[z] = {};
  ...
  const [activeZone,  setActiveZone]  = useS(null);
  const [showBench,   setShowBench]   = useS(false);
  const [activeWType, setActiveWType] = useS('All');
