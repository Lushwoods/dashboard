...
export default function App() {
  ...
  const [logisticsRows,   setLogisticsRows]   = useState([]); // shared from LogisticsView → SoilTracker
  const [logisticsFiles,  setLogisticsFiles]  = useState([]);
  const [logisticsScreen, setLogisticsScreen] = useState('upload');
    ...
    <div className="dash-root" style={{fontFamily:"'Poppins',sans-serif",background:C.bg,minHeight:"100vh",width:"100vw",maxWidth:"100vw",color:C.text,fontSize:13,overflowX:"hidden"}}>
      ...
      <div style={{background:C.nav,borderBottom:`1px solid ${C.border}`,padding:"12px 20px 0 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",paddingBottom:8,borderBottom:`1px solid ${C.border}`}}>
          {tab !== "trucks" && tab !== "logistics" && tab !== "soiltracker" && tab !== "hotel" && <><span style={{color:C.sub,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginRight:4}}>Area:</span>
          {AREA_GROUPS.map(ag=>(
          ...
          {tab === "logistics" && <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:16}}>📦</span><span style={{color:C.sub,fontSize:11,fontWeight:700}}>Logistics Delivery Plan — no zone filter applied</span></div>}
          {tab === "soiltracker" && <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:16}}>🪣</span><span style={{color:C.sub,fontSize:11,fontWeight:700}}>Soil Tracker — Schedule vs KTC Actual</span></div>}
          {tab === "hotel" && <div style={{flex:1,display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:16}}>🏨</span><span style={{color:C.sub,fontSize:11,fontWeight:700}}>Hotel Schedule — T.1 / T.2 / T.3 / T.4 Tower Works</span></div>}
        </div>

        {tab!=="analysis" && tab!=="trucks" && tab!=="logistics" && tab!=="soiltracker" && tab!=="hotel" && <div style={{paddingTop:7,paddingBottom:7,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"flex-start",gap:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",paddingTop:7,paddingBottom:10}}>
          {!zoneDef?.levelFilter && tab!=="analysis" && tab!=="trucks" && tab!=="logistics" && tab!=="soiltracker" && tab!=="hotel" && (() => {
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
            {[["gantt","📊 Gantt",false,null],["analysis","📈 Productivity",false,null],["cards","🃏 Prod Cards",false,null],["resources","📋 Resources",true,null],["scurve","📉 S-Curve",true,null],["settlement","🏦 Settlement",true,null],["health","🏥 Schedule Health",true,null],["hedging","⚖️ Hedging",true,null],["trucks","🚛 Excavation",true,"SmartSite"],["logistics","📦 Logistics",false,null],["soiltracker","🪣 Soil Tracker",false,null],["hotel","🏨 Hotel",false,null]].map(([v,l,playground,badge])=>(
              <div key={v} style={{position:'relative',display:'inline-flex',flexDirection:'column',alignItems:'center',gap:1}}>
                <button onClick={()=>setTab(v)} title={playground?"⚠️ Playground only — do not extract data from this page":undefined} style={{
                  ...
                  {l}
                  {badge && <span style={{fontSize:8,fontWeight:600,opacity:0.75,color:tab===v?'#fff':C.muted,letterSpacing:0.2}}>({badge})</span>}
                </button>
      ...
      {tasks.length === 0 && tab !== "trucks" && tab !== "logistics" && tab !== "soiltracker" && tab !== "hotel" && (
      ...
      {tab==="logistics" && <LogisticsView C={C} lightMode={lightMode}
        rows={logisticsRows}         setRows={setLogisticsRows}
        files={logisticsFiles}       setFiles={setLogisticsFiles}
        screen={logisticsScreen}     setScreen={setLogisticsScreen}
      />}
      {tab==="soiltracker" && <SoilTrackerView tasks={tasks} logisticsRows={logisticsRows} uploadedFile={uploadedFile} logisticsFiles={logisticsFiles} C={C} lightMode={lightMode}/>}
      {tab==="hotel" && <HotelView C={C} lightMode={lightMode}/>}
    </div>
  );
}
...
// ─── HOTEL VIEW ───────────────────────────────────────────────────────────────
const HOTEL_ZONE_COLORS = { 'Zone T.1':'#06b6d4', 'Zone T.2':'#ef4444', 'Zone T.3':'#10b981', 'Zone T.4':'#8b5cf6' };
const HOTEL_WORK_COLORS = {
  'Core Wall':'#6366f1', 'Precast Beam':'#3b82f6', 'Cast In-fill':'#06b6d4',
  'Column & Staircase':'#10b981', 'Raised Slab':'#f59e0b', 'Tower Crane':'#475569',
  'Lift Core':'#8b5cf6', 'Rectification':'#ef4444', 'Other':'#94a3b8',
};

function HotelView({ C, lightMode }) {
  ...
  const [filterFloat, setFilterFloat] = useState('all'); // all | critical | near | float
  const [filterWork,  setFilterWork]  = useState('all');
  const [searchQ,     setSearchQ]     = useState('');
  const [activeView,  setActiveView]  = useState('overview'); // overview | gantt | table
  ...
  async function handleParse(file) {
    ...
    setParsing(true); setParseMsg('Reading hotel schedule…');
    try {
      const buf = await file.arrayBuffer();
      const wb  = window.XLSX.read(buf, { type:'array', cellDates:false, raw:true });
      // Find hotel sheet
      const sheetName = wb.SheetNames.find(n => /hotel/i.test(n)) || wb.SheetNames[0];
      const ws   = wb.Sheets[sheetName];
      const data = window.XLSX.utils.sheet_to_json(ws, { header:1, defval:'', raw:true });

      function ser2date(v) {
        try {
          const n = parseFloat(v);
          if (n > 40000 && n < 62000) {
            const d = new Date(Math.round((n - 25569) * 86400 * 1000) + 43200000);
            return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
          }
        } catch{}
        return null;
      ...
      function classifyWork(name) {
        ...
        if (n.includes('tower crane') || n.includes('shift') && n.includes('crane')) return 'Tower Crane';
        if (n.includes('core wall')) return 'Core Wall';
        if (n.includes('precast beam')) return 'Precast Beam';
        if (n.includes('cast in-fill') || n.includes('in-fill') || n.includes('in-situ')) return 'Cast In-fill';
        if (n.includes('column') || n.includes('staircase')) return 'Column & Staircase';
        if (n.includes('raised slab')) return 'Raised Slab';
        if (n.includes('lift core')) return 'Lift Core';
        if (n.includes('rectif')) return 'Rectification';
        if (n.includes('prepare') || n.includes('setup') || n.includes('erection')) return 'Core Wall';
        return 'Other';
      }

      const acts = [];
      let currentZone = null, currentLevel = null;

      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (!row || row.length < 2) continue;
        const rawId = String(row[0] || '');
        const stripped = rawId.trim();
        const indent   = rawId.length - rawId.trimStart().length;

        if (!stripped) continue;

        if (!stripped.includes('_')) {
          // WBS row
          if (/Hotel Zone T\.\d/.test(stripped) || stripped.startsWith('Zone T.')) currentZone = stripped.replace('Hotel ','');
          else if (/^Level \d+$/.test(stripped) || stripped === 'Roof') currentLevel = stripped;
          continue;
        }

        // Activity row
        const name    = String(row[1] || '').trim();
        const bl_s    = ser2date(row[2]);
        const bl_f    = ser2date(row[3]);
        const bl_dur  = parseInt(row[4]) || 0;
        const sch_pct = parseFloat(row[5]) || 0;
        const act_s   = ser2date(row[6]);
        const act_f   = ser2date(row[7]);
        const orig_dur= parseInt(row[8]) || 0;
        const float_v = parseInt(row[9]) ?? 999;
        const perf_pct= parseFloat(row[10]) || 0;
        const dur_delta = orig_dur - bl_dur;

        const floatCat = float_v === 0 ? 'critical' : float_v <= 14 ? 'near' : 'float';
        const workType = classifyWork(name);
        const desc     = name.replace(/\[[^\]]+\]/g, '').trim();
        const sch_pct_capped = Math.min(100, sch_pct);

        acts.push({
          id: stripped, name, desc, workType,
          zone: currentZone, level: currentLevel,
          bl_s, bl_f, bl_dur,
          act_s, act_f, orig_dur, dur_delta,
          sch_pct: sch_pct_capped, perf_pct, float: float_v, floatCat,
        });
      }

      // Deduplicate by ID — P6 exports sometimes emit two rows per activity
      // (one with dates+float, one with sch_pct). Merge: prefer the row with float data,
      // but take sch_pct from whichever row has it.
      const byId = {};
      for (const a of acts) {
        if (!byId[a.id]) { byId[a.id] = { ...a }; continue; }
        const existing = byId[a.id];
        // Merge: keep better float data, take max sch_pct
        if (a.float < 999 && existing.float === 999) Object.assign(existing, a);
        existing.sch_pct = Math.min(100, Math.max(existing.sch_pct, a.sch_pct));
        if (!existing.act_s && a.act_s) { existing.act_s = a.act_s; existing.act_f = a.act_f; }
        if (!existing.bl_s  && a.bl_s)  { existing.bl_s  = a.bl_s;  existing.bl_f  = a.bl_f;  }
      }
      const dedupedActs = Object.values(byId);

      setHotelTasks(dedupedActs);
      setParseMsg(`✓ ${dedupedActs.length} activities parsed from "${sheetName}" (${acts.length - dedupedActs.length} duplicates removed)`);
      setParsing(false);
  ...
  const filtered = useMemo(() => {
    let t = hotelTasks.filter(a => a.workType !== 'Tower Crane'); // hide crane ops by default for main views
    if (filterZone  !== 'all') t = t.filter(a => a.zone === filterZone);
    if (filterFloat !== 'all') t = t.filter(a => a.floatCat === filterFloat);
    if (filterWork  !== 'all') t = t.filter(a => a.workType === filterWork);
    if (searchQ.trim()) { const q = searchQ.toLowerCase(); t = t.filter(a => a.desc.toLowerCase().includes(q) || (a.level||'').toLowerCase().includes(q)); }
    return t;
  }, [hotelTasks, filterZone, filterFloat, filterWork, searchQ]);

  // ── KPIs ──
  const allActs       = hotelTasks.filter(a => a.workType !== 'Tower Crane');
  const critCount     = allActs.filter(a => a.floatCat === 'critical').length;
  const nearCount     = allActs.filter(a => a.floatCat === 'near').length;
  const inProgress    = allActs.filter(a => a.sch_pct > 0 && a.sch_pct < 100).length;
  const complete      = allActs.filter(a => a.sch_pct >= 100).length;
  const notStarted    = allActs.filter(a => a.sch_pct === 0).length;
  const earliest      = allActs.reduce((m,a) => a.act_s && a.act_s < m ? a.act_s : m, '9999');
  const latest        = allActs.reduce((m,a) => a.act_f && a.act_f > m ? a.act_f : m, '');
  const avgDurDelta   = allActs.filter(a=>a.dur_delta!==0).length > 0
    ? (allActs.reduce((s,a)=>s+a.dur_delta,0)/allActs.length).toFixed(1) : '0';

  // ── Zone × Level grid ──
  const ZONES = ['Zone T.1','Zone T.2','Zone T.3','Zone T.4'];
  const LEVELS = ['Level 5','Level 6','Level 7','Level 8','Level 9','Level 10','Level 11','Level 12','Level 13','Level 14','Level 15','Level 16','Level 17','Roof'];
  const zoneLevel = useMemo(() => {
    const grid = {};
    ZONES.forEach(z => { grid[z] = {}; LEVELS.forEach(l => grid[z][l] = []); });
    allActs.forEach(a => {
      if (a.zone && a.level && grid[a.zone] && grid[a.zone][a.level] !== undefined)
        grid[a.zone][a.level].push(a);
    });
    return grid;
  }, [hotelTasks]);

  // ── Timeline helpers ──
  const TL_START = new Date('2027-01-01').getTime();
  const TL_END   = new Date('2028-10-01').getTime();
  const TL_MS    = TL_END - TL_START;
  const tPct = d => d ? Math.max(0,Math.min(100,(new Date(d).getTime()-TL_START)/TL_MS*100)) : null;
  const bW   = (s,f) => { const p=tPct(s), q=tPct(f); return (p!==null&&q!==null) ? Math.max(0.4,q-p) : 0; };
  const TODAY_P = (Date.now()-TL_START)/TL_MS*100;

  // Month tick marks
  const monthTicks = useMemo(() => {
    const ticks = []; let d = new Date('2027-01-01');
    while (d.getTime() < TL_END) {
      const mNum = getProjectMonth(d);
      ticks.push({ p:(d.getTime()-TL_START)/TL_MS*100, label:d.toLocaleDateString('en-SG',{month:'short'}), isJan:d.getMonth()===0, yr:d.getFullYear(), mNum });
      d = new Date(d.getFullYear(), d.getMonth()+1, 1);
    }
    return ticks;
  }, []);

  const WORK_TYPES = Object.keys(HOTEL_WORK_COLORS).filter(w => w !== 'Tower Crane');
  ...
  if (screen === 'upload') {
    return (
      <div style={{ background:C.bg, minHeight:'60vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', fontFamily:"'Poppins',sans-serif" }}>
        <input ref={fileRef} type="file" accept=".xlsx,.xls" style={{display:'none'}} onChange={e=>{const f=e.target.files[0];if(f){setHotelFile(f);handleParse(f);}}}/>
        <div style={{ width:'100%', maxWidth:560 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:28 }}>
            <div style={{ width:36, height:36, background:'#8b5cf6', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>🏨</div>
            <div>
              <div style={{ fontSize:18, fontWeight:900, color:C.text }}>Hotel <span style={{ color:'#8b5cf6' }}>Schedule</span></div>
              <div style={{ fontSize:11, color:C.sub, marginTop:2, fontFamily:'monospace' }}>P6 Export · T.1 / T.2 / T.3 / T.4 Tower Works · Jan 2027 – Sep 2028</div>
            </div>
          </div>

          <div style={{ border:`1.5px dashed #8b5cf666`, borderRadius:14, padding:'2.5rem 1.5rem', textAlign:'center', cursor:'pointer', background:C.panel, marginBottom:14 }}
            onClick={() => fileRef.current?.click()}
            onDragOver={e=>e.preventDefault()}
            onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f){setHotelFile(f);handleParse(f);}}}>
            <div style={{ fontSize:40, marginBottom:10 }}>📋</div>
            <div style={{ fontSize:15, fontWeight:700, color:C.text, marginBottom:4 }}>Upload P6 Excel Export</div>
            <div style={{ fontSize:11, color:C.sub, fontFamily:'monospace' }}>Looks for a sheet named "hotel" · Activity ID · BL Dates · Current Schedule · Float</div>
          </div>

          {hotelFile && (
            <div style={{ display:'flex', alignItems:'center', gap:8, background:C.panel, border:`1px solid ${C.border}`, borderRadius:8, padding:'8px 14px', marginBottom:12 }}>
              <span style={{ width:8, height:8, borderRadius:'50%', background:'#8b5cf6', flexShrink:0 }}/>
              <span style={{ fontSize:11, color:C.text, fontFamily:'monospace' }}>{hotelFile.name}</span>
            </div>
          )}

          {parsing && (
            <div style={{ height:3, background:C.inputBg, borderRadius:2, overflow:'hidden', marginBottom:8 }}>
              <div style={{ height:'100%', width:'60%', background:'#8b5cf6', borderRadius:2, animation:'indeterminate 1.2s ease infinite' }}/>
            </div>
          )}
          {parseMsg && <div style={{ fontSize:11, color: parseMsg.startsWith('✓')?'#10b981':'#f59e0b', fontFamily:'monospace', marginBottom:10 }}>{parseMsg}</div>}

          <button onClick={()=>fileRef.current?.click()} disabled={parsing||!xlsxReady}
            style={{ width:'100%', padding:'13px', background:'#8b5cf6', color:'#fff', border:'none', borderRadius:12, fontSize:14, fontWeight:800, fontFamily:'inherit', cursor:'pointer', opacity:parsing?0.5:1 }}>
            {parsing ? 'Parsing…' : hotelFile ? 'Re-upload →' : 'Select P6 Excel →'}
          </button>
        </div>
      </div>
    );
  ...
  // ── DASHBOARD ──
  const zoneOpts  = ZONES.filter(z => hotelTasks.some(a=>a.zone===z));
  const floatBadge = (f,cnt) => {
    const cols = { critical:'#ef4444', near:'#f59e0b', float:'#22c55e' };
    return <span style={{ fontSize:9,fontWeight:800,padding:'1px 7px',borderRadius:10,background:cols[f]+'22',color:cols[f],border:`1px solid ${cols[f]}44` }}>{cnt}</span>;
  };
  ...
  return (
    <div style={{ background:C.bg, minHeight:'60vh', padding:'20px 24px', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
            <span style={{ background:'#8b5cf6', color:'#fff', fontSize:10, fontWeight:800, padding:'3px 10px', borderRadius:4, letterSpacing:1, textTransform:'uppercase' }}>Hotel</span>
            <span style={{ color:C.sub, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>Tower Schedule · P6 Export</span>
          </div>
          <div style={{ fontSize:18, fontWeight:900, color:C.text }}>T.1 · T.2 · T.3 · T.4 — Level 5 to Roof</div>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:4, flexWrap:'wrap' }}>
            <span style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>{hotelFile?.name}</span>
            <span style={{ fontSize:9, color:C.muted }}>·</span>
            <span style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>{earliest} → {latest}</span>
            <span style={{ fontSize:9, color:C.muted }}>·</span>
            <span style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>{allActs.length} activities</span>
          </div>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {/* View toggle */}
          <div style={{ display:'flex', gap:3, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
            {[['overview','📊 Overview'],['gantt','📅 Gantt'],['table','📋 Table']].map(([k,l])=>(
              <button key={k} onClick={()=>setActiveView(k)} style={{ padding:'5px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:700, fontFamily:'inherit', background:activeView===k?'#8b5cf6':'transparent', color:activeView===k?'#fff':C.sub }}>{l}</button>
            ))}
          </div>
          <button onClick={()=>{setScreen('upload');setHotelTasks([]);setHotelFile(null);setParseMsg('');}}
            style={{ fontSize:11, color:C.sub, border:`1px solid ${C.border}`, borderRadius:8, padding:'7px 12px', background:'transparent', cursor:'pointer', fontFamily:'inherit' }}>
            ← re-upload
          </button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, marginBottom:16, overflow:'hidden', flexWrap:'wrap' }}>
        {[
          ['Total Activities', allActs.length, '#8b5cf6'],
          ['🔴 Critical (Float=0)', critCount, '#ef4444'],
          ['🟡 Near-Critical (≤14d)', nearCount, '#f59e0b'],
          ['In Progress', inProgress, '#3b82f6'],
          ['Not Started', notStarted, C.sub],
          ['Avg Dur Δ vs BL', `${parseFloat(avgDurDelta)>0?'+':''}${avgDurDelta}d`, parseFloat(avgDurDelta)>0?'#ef4444':parseFloat(avgDurDelta)<0?'#22c55e':C.sub],
        ].map(([l,v,c],i)=>(
          <div key={i} style={{ flex:1, minWidth:100, padding:'10px 14px', borderRight:`1px solid ${C.border}` }}>
            <div style={{ fontSize:9, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.4 }}>{l}</div>
            <div style={{ fontSize:16, fontWeight:900, color:c, marginTop:2, fontFamily:'monospace' }}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:3, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
          {['all',...zoneOpts].map(z=>{
            const col = z==='all'?'#8b5cf6':HOTEL_ZONE_COLORS[z]||'#8b5cf6';
            return <button key={z} onClick={()=>setFilterZone(z)} style={{ padding:'4px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:700, fontFamily:'inherit', background:filterZone===z?col:'transparent', color:filterZone===z?'#fff':C.sub }}>{z==='all'?'All Zones':z}</button>;
          })}
        </div>
        {/* Float filter */}
        <div style={{ display:'flex', gap:3, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
          {[['all','All'],['critical','🔴 Critical'],['near','🟡 Near-Crit'],['float','🟢 Float']].map(([k,l])=>(
            <button key={k} onClick={()=>setFilterFloat(k)} style={{ padding:'4px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:700, fontFamily:'inherit', background:filterFloat===k?'#475569':'transparent', color:filterFloat===k?'#fff':C.sub }}>{l}</button>
          ))}
        </div>
        {/* Work type filter */}
        <select value={filterWork} onChange={e=>setFilterWork(e.target.value)} style={{ padding:'5px 10px', borderRadius:8, border:`1px solid ${C.border}`, background:C.inputBg, color:C.sub, fontSize:10, fontFamily:'inherit', outline:'none', cursor:'pointer' }}>
          <option value="all">All Work Types</option>
          {WORK_TYPES.map(w=><option key={w} value={w}>{w}</option>)}
        </select>
        {/* Search */}
        <input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Search activity…"
          style={{ padding:'5px 12px', borderRadius:8, border:`1px solid ${C.border}`, background:C.inputBg, color:C.text, fontSize:10, fontFamily:'inherit', outline:'none', minWidth:180 }}/>
        <span style={{ marginLeft:'auto', fontSize:10, color:C.muted, fontStyle:'italic' }}>{filtered.length} activities</span>
      </div>

      {/* ══════════ OVERVIEW VIEW ══════════ */}
      {activeView === 'overview' && (
        <div>
          {/* Zone × Level heat grid */}
          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', marginBottom:16 }}>
            <div style={{ padding:'10px 18px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontSize:11, fontWeight:800, color:C.text }}>Zone × Level Progress Grid</span>
              <div style={{ display:'flex', gap:10, fontSize:9, color:C.sub }}>
                {[['#ef4444','Critical'],['#f59e0b','Near-Crit'],['#22c55e','Float OK'],['#3b82f6','In Progress'],['#8b5cf622','Not Started']].map(([col,lbl])=>(
            ...
            <div style={{ overflowX:'auto' }}>
              <table style={{ borderCollapse:'collapse', fontSize:10, minWidth:700 }}>
                <thead>
                  <tr style={{ background:C.bg }}>
                    <th style={{ padding:'6px 14px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:'left', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>Level</th>
                    {ZONES.map(z=>(
                      <th key={z} style={{ padding:'6px 16px', color:HOTEL_ZONE_COLORS[z], fontSize:9, fontWeight:800, textAlign:'center', borderBottom:`1px solid ${C.border}`, borderLeft:`1px solid ${C.border}`, whiteSpace:'nowrap', background:HOTEL_ZONE_COLORS[z]+'11' }}>{z}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {LEVELS.filter(l => ZONES.some(z => (zoneLevel[z]?.[l]||[]).length>0)).map((lvl,ri) => (
                    <tr key={lvl} style={{ borderBottom:`1px solid ${C.border}`, background:ri%2===0?C.tableRow0:C.tableRow1 }}>
                      <td style={{ padding:'5px 14px', fontWeight:700, color:C.text, whiteSpace:'nowrap', fontFamily:'monospace', fontSize:10 }}>{lvl}</td>
                      {ZONES.map(z => {
                        const acts = zoneLevel[z]?.[lvl] || [];
                        if (!acts.length) return <td key={z} style={{ padding:'5px 10px', textAlign:'center', borderLeft:`1px solid ${C.border}`, color:C.muted, fontSize:9 }}>—</td>;
                        const crit    = acts.filter(a=>a.floatCat==='critical').length;
                        const near    = acts.filter(a=>a.floatCat==='near').length;
                        const prog    = acts.filter(a=>a.sch_pct>0).length;
                        const earliest = acts.reduce((m,a)=>a.act_s&&a.act_s<m?a.act_s:m,'9999');
                        const latest   = acts.reduce((m,a)=>a.act_f&&a.act_f>m?a.act_f:m,'');
                        const cellCol  = crit>0?'#ef4444':near>0?'#f59e0b':prog>0?'#3b82f6':'#22c55e';
                        const durD     = acts.reduce((s,a)=>s+a.dur_delta,0);
                        return (
                          <td key={z} style={{ padding:'5px 10px', textAlign:'center', borderLeft:`1px solid ${C.border}`, background:cellCol+'14' }}>
                            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
                              <div style={{ display:'flex', gap:3, justifyContent:'center' }}>
                                {crit>0 && <span style={{ fontSize:8, fontWeight:800, color:'#ef4444', background:'#ef444422', padding:'1px 5px', borderRadius:4 }}>🔴{crit}</span>}
                                {near>0 && <span style={{ fontSize:8, fontWeight:800, color:'#f59e0b', background:'#f59e0b22', padding:'1px 5px', borderRadius:4 }}>🟡{near}</span>}
                                {crit===0&&near===0&&<span style={{ fontSize:8, color:'#22c55e', fontWeight:700 }}>✓ {acts.length}</span>}
                              </div>
                              <div style={{ fontSize:8, color:C.sub, fontFamily:'monospace', whiteSpace:'nowrap' }}>{earliest.slice(5)}→{latest.slice(5)}</div>
                              {durD!==0 && <div style={{ fontSize:8, fontWeight:700, color:durD>0?'#ef4444':'#22c55e' }}>{durD>0?'+':''}{durD}d</div>}
                            </div>
                          </td>
                        );
                      })}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:16 }}>
            {ZONES.filter(z=>hotelTasks.some(a=>a.zone===z)).map(z => {
              const zActs  = allActs.filter(a=>a.zone===z);
              const zCrit  = zActs.filter(a=>a.floatCat==='critical').length;
              const zNear  = zActs.filter(a=>a.floatCat==='near').length;
              const zProg  = zActs.filter(a=>a.sch_pct>0).length;
              const zStart = zActs.reduce((m,a)=>a.act_s&&a.act_s<m?a.act_s:m,'9999');
              const zEnd   = zActs.reduce((m,a)=>a.act_f&&a.act_f>m?a.act_f:m,'');
              const col    = HOTEL_ZONE_COLORS[z];
              const avgDur = zActs.length>0?(zActs.reduce((s,a)=>s+a.dur_delta,0)/zActs.length).toFixed(0):'0';
              return (
                <div key={z} style={{ background:C.panel, border:`1px solid ${col}44`, borderLeft:`3px solid ${col}`, borderRadius:10, padding:'12px 14px' }}>
                  <div style={{ fontSize:12, fontWeight:800, color:col, marginBottom:6 }}>{z}</div>
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:10 }}>
                      <span style={{ color:C.sub }}>Activities</span>
                      <span style={{ color:C.text, fontWeight:700, fontFamily:'monospace' }}>{zActs.length}</span>
                    </div>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:10 }}>
                      <span style={{ color:C.sub }}>Float</span>
                      <span style={{ fontFamily:'monospace' }}>
                        {zCrit>0 && <span style={{color:'#ef4444',fontWeight:700}}>🔴{zCrit} </span>}
                        {zNear>0 && <span style={{color:'#f59e0b',fontWeight:700}}>🟡{zNear} </span>}
                        {zCrit===0&&zNear===0&&<span style={{color:'#22c55e',fontWeight:700}}>✓ All clear</span>}
                      </span>
                    </div>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:10 }}>
                      <span style={{ color:C.sub }}>In Progress</span>
                      <span style={{ color:'#3b82f6', fontWeight:700 }}>{zProg}</span>
                    </div>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:10 }}>
                      <span style={{ color:C.sub }}>Avg Dur Δ</span>
                      <span style={{ color:parseFloat(avgDur)>0?'#ef4444':parseFloat(avgDur)<0?'#22c55e':C.sub, fontWeight:700, fontFamily:'monospace' }}>{parseFloat(avgDur)>0?'+':''}{avgDur}d</span>
                    </div>
                    <div style={{ marginTop:4, paddingTop:4, borderTop:`1px solid ${C.border}`, fontSize:9, color:C.muted, fontFamily:'monospace' }}>{zStart.slice(0,7)} → {zEnd.slice(0,7)}</div>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:16, marginBottom:16 }}>
            <div style={{ fontSize:11, fontWeight:800, color:C.text, marginBottom:12 }}>Work Type Breakdown</div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {WORK_TYPES.map(wt => {
                const cnt  = allActs.filter(a=>a.workType===wt).length;
                const crit = allActs.filter(a=>a.workType===wt&&a.floatCat==='critical').length;
                const maxC = Math.max(...WORK_TYPES.map(w=>allActs.filter(a=>a.workType===w).length),1);
                const col  = HOTEL_WORK_COLORS[wt];
                if (!cnt) return null;
                return (
                  <div key={wt} style={{ display:'grid', gridTemplateColumns:'150px 1fr 60px 60px', alignItems:'center', gap:10 }}>
                    <span style={{ fontSize:10, color:C.sub, textAlign:'right', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={wt}>{wt}</span>
                    <div style={{ background:C.inputBg, borderRadius:3, height:16, overflow:'hidden', position:'relative' }}>
                      <div style={{ width:`${(cnt/maxC)*100}%`, height:'100%', background:col, borderRadius:3, opacity:0.8 }}/>
                      {crit>0&&<div style={{ position:'absolute', right:4, top:0, bottom:0, display:'flex', alignItems:'center' }}>
                        <span style={{ fontSize:8, color:'#ef4444', fontWeight:800 }}>🔴{crit}</span>
                      </div>}
                    </div>
                    <span style={{ fontSize:10, fontWeight:700, color:C.text, fontFamily:'monospace', textAlign:'right' }}>{cnt}</span>
                    <span style={{ fontSize:9, color:C.muted, fontFamily:'monospace', textAlign:'right' }}>{((cnt/allActs.length)*100).toFixed(0)}%</span>
                  </div>
                );
              })}
            </div>
      ...
      {activeView === 'gantt' && (
        <div style={{ fontFamily:"'Poppins',sans-serif" }}>
          ...
          {/* ── LEGEND ── */}
          <div style={{ display:'flex', gap:16, padding:'8px 4px', marginBottom:10, flexWrap:'wrap', alignItems:'center' }}>
            {ZONES.filter(z=>hotelTasks.some(a=>a.zone===z)).map(z=>(
              <div key={z} style={{ display:'flex', alignItems:'center', gap:6 }}>
                <div style={{ width:20, height:8, borderRadius:3, background:HOTEL_ZONE_COLORS[z] }}/>
                <span style={{ fontSize:10, fontWeight:700, color:HOTEL_ZONE_COLORS[z] }}>{z}</span>
              </div>
            ))}
            <div style={{ display:'flex', alignItems:'center', gap:6, marginLeft:8, paddingLeft:8, borderLeft:`1px solid ${C.border}` }}>
              <div style={{ width:20, height:4, borderRadius:2, background:C.muted, opacity:0.5 }}/>
              <span style={{ fontSize:9, color:C.sub }}>Baseline</span>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <div style={{ width:20, height:7, borderRadius:2, background:'#8b5cf6', opacity:0.85 }}/>
              <span style={{ fontSize:9, color:C.sub }}>Current schedule</span>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <div style={{ width:12, height:7, borderRadius:2, background:'#22c55e' }}/>
              <span style={{ fontSize:9, color:C.sub }}>% complete</span>
            </div>
            <span style={{ marginLeft:'auto', fontSize:9, color:C.muted, fontStyle:'italic' }}>{filtered.filter(a=>a.workType!=='Tower Crane').length} activities</span>
          </div>

          {/* ── SUMMARY GANTT — one row per Zone × Level ── */}
          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', marginBottom:12 }}>
            <div style={{ padding:'8px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontSize:11, fontWeight:800, color:C.text }}>Summary — Zone × Level</span>
              <span style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>Each bar = earliest start → latest finish across all activities in that level</span>
            </div>
            {/* Timeline header */}
            <div style={{ display:'flex', background:C.bg, borderBottom:`1px solid ${C.border}`, minWidth:'max-content', position:'sticky', top:0, zIndex:8 }}>
              <div style={{ width:200, minWidth:200, flexShrink:0, padding:'6px 14px', fontSize:9, color:C.sub, fontWeight:700, textTransform:'uppercase', borderRight:`1px solid ${C.border}` }}>Zone · Level</div>
              <div style={{ flex:1, minWidth:700, position:'relative', height:34 }}>
                {monthTicks.map((t,i)=>(
                  <div key={i} style={{ position:'absolute', left:`${t.p}%`, top:0, bottom:0, borderLeft:`1px solid ${C.muted}33`, paddingLeft:3, display:'flex', flexDirection:'column', justifyContent:'center', gap:1 }}>
                    <span style={{ fontSize:7, color:'#3b82f6', fontWeight:800, lineHeight:1, whiteSpace:'nowrap' }}>M{t.mNum}</span>
                    <span style={{ fontSize:t.isJan?8:7, color:t.isJan?C.text:C.sub, fontWeight:t.isJan?700:400, lineHeight:1, whiteSpace:'nowrap' }}>{t.isJan?`${t.label} '${String(t.yr).slice(2)}`:t.label}</span>
                  </div>
                ))}
                {TODAY_P>0&&TODAY_P<100&&<div style={{ position:'absolute', left:`${TODAY_P}%`, top:0, bottom:0, width:2, background:C.amber, zIndex:5 }}/>}
              </div>
            </div>
            {/* Summary rows */}
            <div style={{ overflowX:'auto', maxHeight:420 }}>
              {ZONES.filter(z=>allActs.some(a=>a.zone===z)).map(zone=>{
                const zcol = HOTEL_ZONE_COLORS[zone];
                const levels = LEVELS.filter(lvl => allActs.some(a=>a.zone===zone&&a.level===lvl));
                return (
                  <div key={zone}>
                    {/* Zone header row */}
                    <div style={{ display:'flex', minWidth:'max-content', background:zcol+'22', borderBottom:`1px solid ${zcol}44`, borderTop:`1px solid ${zcol}33` }}>
                      <div style={{ width:200, minWidth:200, padding:'5px 14px', color:zcol, fontSize:11, fontWeight:900, borderRight:`1px solid ${C.border}`, background:zcol+'33', letterSpacing:0.5 }}>{zone}</div>
                      <div style={{ flex:1, minWidth:700, position:'relative', height:28 }}>
                        {/* Zone-wide span bar */}
                        {(() => {
                          const zA = allActs.filter(a=>a.zone===zone);
                          const zs = zA.reduce((m,a)=>a.act_s&&a.act_s<m?a.act_s:m,'9999');
                          const ze = zA.reduce((m,a)=>a.act_f&&a.act_f>m?a.act_f:m,'');
                          const bs = zA.reduce((m,a)=>a.bl_s&&a.bl_s<m?a.bl_s:m,'9999');
                          const be = zA.reduce((m,a)=>a.bl_f&&a.bl_f>m?a.bl_f:m,'');
                          return <>
                            {bs!=='9999'&&be&&<div style={{ position:'absolute', left:`${tPct(bs)}%`, width:`${bW(bs,be)}%`, top:5, height:5, background:zcol, borderRadius:2, opacity:0.25 }}/>}
                            {zs!=='9999'&&ze&&<div style={{ position:'absolute', left:`${tPct(zs)}%`, width:`${bW(zs,ze)}%`, top:14, height:8, background:zcol, borderRadius:3, opacity:0.6 }}>
                              <span style={{ position:'absolute', left:4, top:0, bottom:0, display:'flex', alignItems:'center', fontSize:7, color:'#fff', fontWeight:700, whiteSpace:'nowrap' }}>{zs.slice(0,7)}→{ze.slice(0,7)}</span>
                            </div>}
                            {TODAY_P>0&&TODAY_P<100&&<div style={{ position:'absolute', left:`${TODAY_P}%`, top:0, bottom:0, width:1, background:C.amber+'88' }}/>}
                          </>;
                        })()}
                      </div>
                    </div>
                    {/* Level rows */}
                    {levels.map((lvl,li)=>{
                      const lA   = allActs.filter(a=>a.zone===zone&&a.level===lvl);
                      const ls   = lA.reduce((m,a)=>a.act_s&&a.act_s<m?a.act_s:m,'9999');
                      const le   = lA.reduce((m,a)=>a.act_f&&a.act_f>m?a.act_f:m,'');
                      const bls  = lA.reduce((m,a)=>a.bl_s&&a.bl_s<m?a.bl_s:m,'9999');
                      const ble  = lA.reduce((m,a)=>a.bl_f&&a.bl_f>m?a.bl_f:m,'');
                      const crit = lA.filter(a=>a.floatCat==='critical').length;
                      const near = lA.filter(a=>a.floatCat==='near').length;
                      const pct  = lA.filter(a=>a.sch_pct>0&&a.sch_pct<=100).length;
                      const workActs = lA.filter(a=>a.workType!=='Tower Crane'&&a.sch_pct<=100);
                      const avgPct = workActs.length>0?(workActs.reduce((s,a)=>s+a.sch_pct,0)/workActs.length).toFixed(0):0;
                      const rowBg = li%2===0?C.tableRow0:C.tableRow1;
                      return (
                        <div key={lvl} style={{ display:'flex', minWidth:'max-content', borderBottom:`1px solid ${C.border}`, height:30, alignItems:'center', background:rowBg }}
                          onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                          onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                          <div style={{ width:200, minWidth:200, padding:'0 14px 0 22px', display:'flex', alignItems:'center', justifyContent:'space-between', borderRight:`1px solid ${C.border}`, gap:6 }}>
                            <span style={{ fontSize:10, fontWeight:700, color:zcol }}>{lvl}</span>
                            <div style={{ display:'flex', gap:3, alignItems:'center', flexShrink:0 }}>
                              {crit>0&&<span style={{ fontSize:8, color:'#ef4444', fontWeight:800, background:'#ef444422', padding:'1px 4px', borderRadius:3 }}>🔴{crit}</span>}
                              {near>0&&<span style={{ fontSize:8, color:'#f59e0b', fontWeight:800, background:'#f59e0b22', padding:'1px 4px', borderRadius:3 }}>🟡{near}</span>}
                              {pct>0&&<span style={{ fontSize:8, color:'#22c55e', fontWeight:800 }}>{avgPct}%</span>}
                            </div>
                          </div>
                          <div style={{ flex:1, position:'relative', height:'100%', minWidth:700 }}>
                            {bls!=='9999'&&ble&&<div style={{ position:'absolute', left:`${tPct(bls)}%`, width:`${bW(bls,ble)}%`, top:4, height:5, background:zcol, borderRadius:2, opacity:0.22 }}/>}
                            {ls!=='9999'&&le&&<div style={{ position:'absolute', left:`${tPct(ls)}%`, width:`${bW(ls,le)}%`, top:13, height:10, background:zcol, borderRadius:3, opacity:0.85 }}>
                              {parseFloat(avgPct)>0&&<div style={{ width:`${avgPct}%`, height:'100%', background:'#22c55e', borderRadius:3, opacity:0.9 }}/>}
                            </div>}
                            {TODAY_P>0&&TODAY_P<100&&<div style={{ position:'absolute', left:`${TODAY_P}%`, top:0, bottom:0, width:1, background:C.amber+'66' }}/>}
                          </div>
                        </div>
                      );
                    })}
          ...
          {/* ── DETAIL GANTT — individual activities, zone coloured ── */}
          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'8px 16px', borderBottom:`1px solid ${C.border}` }}>
              <span style={{ fontSize:11, fontWeight:800, color:C.text }}>Detail — Individual Activities</span>
              <span style={{ fontSize:9, color:C.sub, marginLeft:10 }}>Tower Crane rows hidden · filters applied</span>
            </div>
            {/* Timeline header */}
            <div style={{ display:'flex', background:C.bg, borderBottom:`1px solid ${C.border}`, minWidth:'max-content', position:'sticky', top:0, zIndex:8 }}>
              <div style={{ width:260, minWidth:260, flexShrink:0, padding:'6px 14px', fontSize:9, color:C.sub, fontWeight:700, textTransform:'uppercase', borderRight:`1px solid ${C.border}` }}>Activity</div>
              <div style={{ flex:1, minWidth:700, position:'relative', height:34 }}>
                {monthTicks.map((t,i)=>(
                  <div key={i} style={{ position:'absolute', left:`${t.p}%`, top:0, bottom:0, borderLeft:`1px solid ${C.muted}33`, paddingLeft:3, display:'flex', flexDirection:'column', justifyContent:'center', gap:1 }}>
                    <span style={{ fontSize:7, color:'#3b82f6', fontWeight:800, lineHeight:1, whiteSpace:'nowrap' }}>M{t.mNum}</span>
            ...
            {/* Rows */}
            <div style={{ overflowY:'auto', maxHeight:520, overflowX:'auto', minWidth:'max-content' }}>
              {ZONES.map(zone=>{
                const zActs = filtered.filter(a=>a.zone===zone&&a.workType!=='Tower Crane');
                if (!zActs.length) return null;
                const zcol = HOTEL_ZONE_COLORS[zone];
                return (
                  <div key={zone}>
                    {/* Zone header */}
                    <div style={{ display:'flex', background:zcol+'1a', borderBottom:`1px solid ${zcol}33`, borderTop:`1px solid ${zcol}22`, position:'sticky', top:0, zIndex:4 }}>
                      <div style={{ width:260, minWidth:260, padding:'5px 14px', color:zcol, fontSize:10, fontWeight:900, textTransform:'uppercase', borderRight:`1px solid ${C.border}`, background:zcol+'28', letterSpacing:0.5 }}>
                        {zone} <span style={{ fontSize:8, fontWeight:600, opacity:0.7 }}>· {zActs.length} acts</span>
                      </div>
                      <div style={{ flex:1, minWidth:700, position:'relative', height:26 }}>
                        {/* Thin zone span line */}
                        {(()=>{
                          const zs=zActs.reduce((m,a)=>a.act_s&&a.act_s<m?a.act_s:m,'9999');
                          const ze=zActs.reduce((m,a)=>a.act_f&&a.act_f>m?a.act_f:m,'');
                          return zs!=='9999'&&ze&&<div style={{ position:'absolute', left:`${tPct(zs)}%`, width:`${bW(zs,ze)}%`, top:10, height:3, background:zcol, borderRadius:2, opacity:0.35 }}/>;
                        })()}
                        {TODAY_P>0&&TODAY_P<100&&<div style={{ position:'absolute', left:`${TODAY_P}%`, top:0, bottom:0, width:1, background:C.amber+'55' }}/>}
                      </div>
                    </div>
                    {/* Activity rows */}
                    {LEVELS.map(lvl=>{
                      const lActs = zActs.filter(a=>a.level===lvl);
                      if (!lActs.length) return null;
                      return lActs.map((a,ri)=>{
                        const floatCol = a.floatCat==='critical'?'#ef4444':a.floatCat==='near'?'#f59e0b':'#22c55e';
                        const rowBg    = ri%2===0?C.tableRow0:C.tableRow1;
                        return (
                          <div key={a.id} style={{ display:'flex', borderBottom:`1px solid ${C.border}`, height:27, alignItems:'center', background:rowBg }}
                            onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                            onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                            <div style={{ width:260, minWidth:260, padding:'0 10px', display:'flex', alignItems:'center', gap:5, borderRight:`1px solid ${C.border}`, overflow:'hidden', borderLeft:`3px solid ${zcol}` }}>
                              <span style={{ fontSize:8, color:floatCol, fontWeight:800, flexShrink:0 }}>{a.floatCat==='critical'?'🔴':a.floatCat==='near'?'🟡':'🟢'}</span>
                              <span style={{ fontSize:8, color:zcol, fontWeight:800, flexShrink:0, opacity:0.8 }}>[{(a.level||'').replace('Level ','')}]</span>
                              <span style={{ fontSize:9, color:C.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={a.desc}>{a.desc}</span>
                            </div>
                            <div style={{ flex:1, position:'relative', height:'100%', minWidth:700 }}>
                              {/* BL bar — muted zone colour */}
                              {a.bl_s&&a.bl_f&&<div style={{ position:'absolute', left:`${tPct(a.bl_s)}%`, width:`${bW(a.bl_s,a.bl_f)}%`, top:4, height:5, background:zcol, borderRadius:2, opacity:0.22 }}/>}
                              {/* Current bar — full zone colour */}
                              {a.act_s&&a.act_f&&<div style={{ position:'absolute', left:`${tPct(a.act_s)}%`, width:`${bW(a.act_s,a.act_f)}%`, top:13, height:9, background:zcol, borderRadius:3, opacity:0.85 }}>
                                {a.sch_pct>0&&<div style={{ width:`${a.sch_pct}%`, height:'100%', background:'#22c55e', borderRadius:3, opacity:0.9 }}/>}
                              </div>}
                              {TODAY_P>0&&TODAY_P<100&&<div style={{ position:'absolute', left:`${TODAY_P}%`, top:0, bottom:0, width:1, background:C.amber+'55' }}/>}
                            </div>
                          </div>
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden' }}>
          <div style={{ overflowX:'auto', maxHeight:600, overflowY:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg, position:'sticky', top:0, zIndex:5 }}>
                  {['Zone','Level','Activity','Work Type','BL Start','BL Finish','BL Dur','Curr Start','Curr Finish','Curr Dur','Dur Δ','Float','Sch %','Status'].map(h=>(
                    <th key={h} style={{ padding:'7px 10px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:h==='Activity'?'left':'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((a,i) => {
                  const zcol   = HOTEL_ZONE_COLORS[a.zone] || '#8b5cf6';
                  const wcol   = HOTEL_WORK_COLORS[a.workType] || '#94a3b8';
                  const fcol   = a.floatCat==='critical'?'#ef4444':a.floatCat==='near'?'#f59e0b':'#22c55e';
                  const dcol   = a.dur_delta>0?'#ef4444':a.dur_delta<0?'#22c55e':C.sub;
                  const rowBg  = i%2===0?C.tableRow0:C.tableRow1;
                  const status = a.sch_pct>=100?'Done':a.sch_pct>0?'In Progress':a.floatCat==='critical'?'Critical':'Not Started';
                  const scol   = a.sch_pct>=100?'#22c55e':a.sch_pct>0?'#3b82f6':a.floatCat==='critical'?'#ef4444':C.sub;
                  return (
                    <tr key={a.id} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                      onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                      onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                      <td style={{ padding:'5px 10px', textAlign:'center' }}><span style={{ fontSize:9, fontWeight:700, color:zcol, background:zcol+'18', padding:'1px 6px', borderRadius:10, whiteSpace:'nowrap' }}>{a.zone}</span></td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:C.sub, fontSize:10, fontWeight:700, fontFamily:'monospace' }}>{a.level}</td>
                      <td style={{ padding:'5px 10px', color:C.text, fontSize:10, maxWidth:240, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={a.desc}>{a.desc}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center' }}><span style={{ fontSize:9, fontWeight:700, color:wcol, background:wcol+'18', padding:'1px 6px', borderRadius:10, whiteSpace:'nowrap' }}>{a.workType}</span></td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:C.sub, fontSize:9, fontFamily:'monospace' }}>{a.bl_s||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:C.sub, fontSize:9, fontFamily:'monospace' }}>{a.bl_f||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:C.text, fontFamily:'monospace', fontSize:10 }}>{a.bl_dur||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:'#3b82f6', fontSize:9, fontFamily:'monospace' }}>{a.act_s||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:'#3b82f6', fontSize:9, fontFamily:'monospace' }}>{a.act_f||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', color:C.text, fontFamily:'monospace', fontSize:10 }}>{a.orig_dur||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center', fontWeight:800, fontFamily:'monospace', color:dcol, fontSize:10 }}>{a.dur_delta>0?'+':''}{a.dur_delta||'—'}</td>
                      <td style={{ padding:'5px 10px', textAlign:'center' }}><span style={{ fontSize:9, fontWeight:800, color:fcol }}>{a.floatCat==='critical'?'🔴 0':a.floatCat==='near'?`🟡 ${a.float}`:`🟢 ${a.float}`}</span></td>
                      <td style={{ padding:'5px 10px', textAlign:'center' }}>
                        {a.sch_pct>0?<div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
                          <span style={{ fontSize:10, fontWeight:700, color:'#3b82f6' }}>{a.sch_pct}%</span>
                          <div style={{ width:40, height:3, background:C.inputBg, borderRadius:2, overflow:'hidden' }}>
                            <div style={{ width:`${a.sch_pct}%`, height:'100%', background:'#3b82f6', borderRadius:2 }}/>
                          </div>
                        </div>:<span style={{ color:C.muted, fontSize:9 }}>—</span>}
                      </td>
                      <td style={{ padding:'5px 10px', textAlign:'center' }}><span style={{ fontSize:9, fontWeight:700, color:scol, background:scol+'18', padding:'1px 6px', borderRadius:10, whiteSpace:'nowrap' }}>{status}</span></td>
                    </tr>
                  );
                })}
                {filtered.length===0&&<tr><td colSpan={14} style={{ padding:'32px', textAlign:'center', color:C.muted, fontStyle:'italic' }}>No activities match this filter</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
...
// ─── SOIL TRACKER VIEW ────────────────────────────────────────────────────────
const KTC_VOL_PER_TRIP = 8;

function SoilTrackerView({ tasks, logisticsRows, uploadedFile, logisticsFiles, C, lightMode }) {
  ...
  // ── Planned excavation — use pcClassify exactly like Prod Cards do ──
  // This ensures total matches what Prod Cards show for the 'excavate' work type
  const plannedData = useMemo(() =>
    tasks.filter(t => pcClassify(t.activity) === 'excavate'),
  [tasks]);

  // ── KTC actual soil movement ──
  const ktcRows = useMemo(() =>
    logisticsRows.filter(r => r.subcon.includes('KTC') && r.vtype.includes('TIPPER')),
  [logisticsRows]);
  ...
  function keyOf(d) { return groupBy === 'month' ? monthOf(d) : quarterOf(d); }
  function labelOf(k) {
    if (!k) return '—';
    if (groupBy === 'month') {
      const d = new Date(k+'-15');
      return isNaN(d) ? k : d.toLocaleDateString('en-SG',{month:'short',year:'numeric'});
    }
    return k;
  }

  // ── Aggregate planned vol using daily rate ──
  // Rate = cc_qty ÷ cc_dur (m³/day), then × actual days overlapping each bucket
  // This is the correct approach: e.g. 23,703 m³ ÷ 35d = 677 m³/day
  const plannedByBucket = useMemo(() => {
    const m = {};

    plannedData.forEach(t => {
      const vol    = t.cc_qty || t.volume || 0;
      const durDays = t.cc_dur || 0;
      if (!vol || !t.cc_s || !t.cc_e) {
        // No duration/dates — fall back to bucketing by cc_e as a lump sum
        const k = keyOf(t.cc_e);
        if (k && vol) {
          if (!m[k]) m[k] = { planned:0, rate:0, tasks:[] };
          m[k].planned += vol;
          m[k].tasks.push(t);
        }
        return;
      ...
      const dailyRate = durDays > 0 ? vol / durDays : 0;
      const ccStart = new Date(t.cc_s);
      const ccEnd   = new Date(t.cc_e);
      if (isNaN(ccStart) || isNaN(ccEnd)) return;

      // Walk each calendar day in cc_s→cc_e, accumulate into correct bucket
      // For performance, walk month by month computing overlap days
      const cur = new Date(ccStart.getFullYear(), ccStart.getMonth(), 1);
      const endMo = new Date(ccEnd.getFullYear(), ccEnd.getMonth(), 1);

      while (cur <= endMo) {
        // Month boundary
        const moStart = new Date(cur.getFullYear(), cur.getMonth(), 1);
        const moEnd   = new Date(cur.getFullYear(), cur.getMonth() + 1, 0); // last day of month

        // Overlap of [cc_s, cc_e] with this month
        const overlapStart = ccStart > moStart ? ccStart : moStart;
        const overlapEnd   = ccEnd   < moEnd   ? ccEnd   : moEnd;
        const overlapDays  = Math.max(0, (overlapEnd - overlapStart) / 86400000 + 1);

        if (overlapDays > 0) {
          // Use mid-month date to get the correct bucket key
          const ds = `${cur.getFullYear()}-${String(cur.getMonth()+1).padStart(2,'0')}-15`;
          const k  = keyOf(ds);
          if (k) {
            if (!m[k]) m[k] = { planned:0, rate:0, tasks:[] };
            m[k].planned += dailyRate * overlapDays;
            m[k].rate    += dailyRate;
            if (!m[k].tasks.includes(t)) m[k].tasks.push(t);
          }
        }
  ...
  const actualByBucket = useMemo(() => {
    ...
    ktcRows.forEach(r => {
      ...
      if (!k) return;
      if (!m[k]) m[k] = 0;
      m[k] += (r.trips || 0) * KTC_VOL_PER_TRIP;
    });
    return m;
  }, [ktcRows, groupBy]);

  const allKeys = useMemo(() => {
    const s = new Set([...Object.keys(plannedByBucket), ...Object.keys(actualByBucket)]);
    return [...s].filter(Boolean).sort();
  }, [plannedByBucket, actualByBucket]);

  const totalPlanned = plannedData.reduce((s,t) => s + (t.cc_qty || t.volume || 0), 0);
  const totalActual  = ktcRows.reduce((s,r) => s+(r.trips||0)*KTC_VOL_PER_TRIP, 0);
  const achievement  = totalPlanned > 0 ? ((totalActual/totalPlanned)*100).toFixed(1) : null;
  const achCol       = achievement === null ? C.sub : parseFloat(achievement)>=80?'#22c55e':parseFloat(achievement)>=50?'#f59e0b':'#ef4444';

  // ── Chart build ──
  function buildChart() {
    if (!window.Chart || !chartRef.current || !allKeys.length) return;
    if (chartInst.current) chartInst.current.destroy();
    const gridCol = lightMode ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.05)';
    const tickCol = lightMode ? '#64748b' : '#6b7280';
    chartInst.current = new window.Chart(chartRef.current, {
      type: 'bar',
      data: {
        labels: allKeys.map(labelOf),
        datasets: [
          {
            label: 'Planned Excavation (m³)',
            data: allKeys.map(k => Math.round(plannedByBucket[k]?.planned||0)),
            backgroundColor: 'rgba(99,102,241,0.4)',
            borderColor: '#6366f1', borderWidth:1, borderRadius:4, borderSkipped:false,
          },
          {
            label: 'KTC Actual (m³)',
            data: allKeys.map(k => Math.round(actualByBucket[k]||0)),
            backgroundColor: 'rgba(251,146,60,0.5)',
            borderColor: '#fb923c', borderWidth:1, borderRadius:4, borderSkipped:false,
          },
        ],
      },
      options: {
        ...
        plugins: {
          legend:{ display:true, labels:{ color:tickCol, font:{size:10,family:"'Poppins',sans-serif"}, boxWidth:12, padding:16 } },
          tooltip:{
            callbacks:{
              label: ctx => `${ctx.dataset.label}: ${Math.round(ctx.raw).toLocaleString()} m³`,
              afterBody: items => {
                const p = items.find(i=>i.datasetIndex===0)?.raw||0;
                const a = items.find(i=>i.datasetIndex===1)?.raw||0;
                if (!p && !a) return [];
                const pct = p>0?((a/p)*100).toFixed(0)+'%': a>0?'Unplanned':'—';
                const delta = Math.round(a-p);
                return [`Achievement: ${pct}`,`Delta: ${delta>=0?'+':''}${delta.toLocaleString()} m³`];
              },
            },
          },
        },
        scales:{
          ...
          y:{ grid:{color:gridCol}, ticks:{color:tickCol,font:{size:10},callback:n=>n>=1000?`${(n/1000).toFixed(0)}k`:n},
            title:{display:true,text:'Volume (m³)',color:C.sub,font:{size:9,weight:'700'}}, beginAtZero:true },
        },
      },
    });
  }
  ...
  useEffect(() => {
    if (!chartJsReady || !allKeys.length) return;
    if (chartRef.current) { buildChart(); return; }
    const raf = requestAnimationFrame(() => {
      if (chartRef.current) buildChart();
      else setTimeout(buildChart, 80);
    });
    return () => cancelAnimationFrame(raf);
  }, [allKeys, chartJsReady, lightMode, groupBy]);

  const noSchedule  = tasks.length === 0;
  const noLogistics = logisticsRows.length === 0;
  ...
  return (
    <div style={{ background:C.bg, minHeight:'60vh', padding:'20px 24px', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={{ marginBottom:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
          <span style={{ background:'#6366f1', color:'#fff', fontSize:10, fontWeight:800, padding:'3px 10px', borderRadius:4, letterSpacing:1, textTransform:'uppercase' }}>Soil Tracker</span>
          <span style={{ color:C.sub, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>Planned vs Actual Soil Movement</span>
        </div>
        <div style={{ fontSize:18, fontWeight:900, color:C.text }}>Excavation Schedule vs KTC Tipper Actuals</div>
        {/* Source file chips */}
        <div style={{ display:'flex', gap:8, marginTop:6, flexWrap:'wrap', alignItems:'center' }}>
          <span style={{ fontSize:9, color:C.sub, fontWeight:700, textTransform:'uppercase', letterSpacing:0.5 }}>Data from:</span>
          {/* Schedule Excel */}
          <div style={{ display:'flex', alignItems:'center', gap:5, background:tasks.length>0?'#6366f118':C.inputBg, border:`1px solid ${tasks.length>0?'#6366f144':C.border}`, borderRadius:6, padding:'3px 10px' }}>
            <span style={{ width:6, height:6, borderRadius:'50%', background:tasks.length>0?'#6366f1':C.muted, flexShrink:0 }}/>
            <span style={{ fontSize:10, color:tasks.length>0?'#a5b4fc':C.muted, fontFamily:'monospace' }}>
              {tasks.length>0 && uploadedFile ? uploadedFile.name : 'No schedule loaded'}
            </span>
            <span style={{ fontSize:9, color:tasks.length>0?'#6366f1':C.muted, fontWeight:700 }}>
              {tasks.length>0 ? `· ${tasks.length} tasks` : ''}
            </span>
          </div>
          {/* Logistics Excel */}
          <div style={{ display:'flex', alignItems:'center', gap:5, background:logisticsRows.length>0?'#fb923c18':C.inputBg, border:`1px solid ${logisticsRows.length>0?'#fb923c44':C.border}`, borderRadius:6, padding:'3px 10px' }}>
            <span style={{ width:6, height:6, borderRadius:'50%', background:logisticsRows.length>0?'#fb923c':C.muted, flexShrink:0 }}/>
            <span style={{ fontSize:10, color:logisticsRows.length>0?'#fdba74':C.muted, fontFamily:'monospace' }}>
              {logisticsRows.length>0 && logisticsFiles?.length>0
                ? logisticsFiles.map(f=>f.name).join(', ')
                : 'No logistics loaded'}
            </span>
            <span style={{ fontSize:9, color:logisticsRows.length>0?'#fb923c':C.muted, fontWeight:700 }}>
              {logisticsRows.length>0 ? `· ${logisticsRows.length.toLocaleString()} records` : ''}
            </span>
          </div>
        </div>
      </div>
      ...
      {(noSchedule || noLogistics) && (
        <div style={{ display:'flex', gap:12, marginBottom:16, flexWrap:'wrap' }}>
          {noSchedule && (
            <div style={{ flex:1, minWidth:240, background:'#6366f122', border:`1px dashed #6366f1`, borderRadius:10, padding:'12px 16px', display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontSize:20 }}>📊</span>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'#6366f1' }}>No Schedule Excel loaded</div>
                <div style={{ fontSize:10, color:C.sub }}>Upload the schedule Excel (Gantt tab) to see planned excavation volumes</div>
              </div>
            </div>
          )}
          {noLogistics && (
            <div style={{ flex:1, minWidth:240, background:'#fb923c22', border:`1px dashed #fb923c`, borderRadius:10, padding:'12px 16px', display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontSize:20 }}>📦</span>
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:'#fb923c' }}>No Logistics data loaded</div>
                <div style={{ fontSize:10, color:C.sub }}>Go to Logistics tab and upload the Delivery Plan Excel first</div>
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ display:'flex', background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, marginBottom:16, overflow:'hidden', flexWrap:'wrap' }}>
        {[
          ['Planned Excav. Vol',  totalPlanned>0?`${Math.round(totalPlanned).toLocaleString()} m³`:'—', '#6366f1'],
          ['KTC Actual Vol',      totalActual >0?`${Math.round(totalActual).toLocaleString()} m³` :'—', '#fb923c'],
          ['Delta',               totalPlanned>0&&totalActual>0
            ? `${Math.round(totalActual-totalPlanned)>=0?'+':''}${Math.round(totalActual-totalPlanned).toLocaleString()} m³`:'—',
            totalActual>=totalPlanned?'#22c55e':'#ef4444'],
          ['Achievement',         achievement?`${achievement}%`:'—', achCol],
          ['Excav. Tasks matched',`${plannedData.length} tasks`, plannedData.length>0?'#6366f1':C.muted],
          ['KTC Trip Days',       ktcRows.length>0?`${new Set(ktcRows.map(r=>r.date)).size} days`:'—', '#fb923c'],
        ].map(([l,v,c],i) => (
          <div key={i} style={{ flex:1, minWidth:110, padding:'10px 14px', borderRight:`1px solid ${C.border}` }}>
            <div style={{ fontSize:9, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.5 }}>{l}</div>
            <div style={{ fontSize:15, fontWeight:900, color:c, marginTop:2, fontFamily:'monospace' }}>{v}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:14, alignItems:'center', flexWrap:'wrap' }}>
        <span style={{ fontSize:10, color:C.sub, fontWeight:700, textTransform:'uppercase' }}>Group by:</span>
        <label style={{ display:'flex', alignItems:'center', gap:7, cursor:'pointer', fontSize:10, color:C.sub, fontWeight:700 }}>
          <div onClick={() => setShowZones(v=>!v)} style={{ width:32, height:16, borderRadius:999, background:showZones?'#6366f144':C.muted, border:`1px solid ${showZones?'#6366f1':C.muted}`, position:'relative', transition:'all 0.2s' }}>
            <div style={{ position:'absolute', top:2, left:showZones?14:2, width:12, height:12, borderRadius:999, background:showZones?'#6366f1':'#475569', transition:'left 0.2s' }}/>
          </div>
          Zone breakdown
        </label>
        <label style={{ display:'flex', alignItems:'center', gap:7, cursor:'pointer', fontSize:10, color:C.sub, fontWeight:700 }}>
          <div onClick={() => setShowDebug(v=>!v)} style={{ width:32, height:16, borderRadius:999, background:showDebug?'#f59e0b44':C.muted, border:`1px solid ${showDebug?'#f59e0b':C.muted}`, position:'relative', transition:'all 0.2s' }}>
            <div style={{ position:'absolute', top:2, left:showDebug?14:2, width:12, height:12, borderRadius:999, background:showDebug?'#f59e0b':'#475569', transition:'left 0.2s' }}/>
          </div>
          Debug
        </label>
        <div style={{ marginLeft:'auto', fontSize:10, color:C.muted, fontFamily:'monospace', fontStyle:'italic' }}>
          ⚠️ KPI total = raw cc_qty (matches Prod Cards) · Chart bars = daily rate × days/period · KTC = trips × 8 m³
        </div>
      </div>

      {/* ── DEBUG PANEL ── */}
      {showDebug && (
        <div style={{ background:C.panel, border:`1px solid #f59e0b55`, borderLeft:`3px solid #f59e0b`, borderRadius:10, padding:16, marginBottom:16, fontSize:10, fontFamily:'monospace' }}>
          <div style={{ fontWeight:800, color:'#f59e0b', marginBottom:10, fontSize:11 }}>🔍 Debug — Schedule Task Matching</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:6, textTransform:'uppercase', letterSpacing:0.5 }}>Total tasks loaded: {tasks.length}</div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:4 }}>Matched as excavation: <span style={{ color:'#6366f1', fontWeight:900 }}>{plannedData.length}</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:4 }}>With cc_qty &gt; 0: <span style={{ color:'#22c55e', fontWeight:900 }}>{plannedData.filter(t=>(t.cc_qty||0)>0).length}</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:4 }}>With volume &gt; 0: <span style={{ color:'#22c55e', fontWeight:900 }}>{plannedData.filter(t=>(t.volume||0)>0).length}</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:4 }}>With cc_s + cc_e + cc_dur: <span style={{ color:'#22c55e', fontWeight:900 }}>{plannedData.filter(t=>t.cc_s&&t.cc_e&&t.cc_dur>0).length}</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:2 }}>Total raw vol (= Prod Cards): <span style={{ color:'#6366f1', fontWeight:900 }}>{Math.round(totalPlanned).toLocaleString()} m³</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:8 }}>Rate-spread vol (chart bars): <span style={{ color:'#a78bfa', fontWeight:900 }}>{Math.round(Object.values(plannedByBucket).reduce((s,v)=>s+v.planned,0)).toLocaleString()} m³</span></div>
              <div style={{ color:C.sub, fontWeight:700, marginBottom:4 }}>Sample matched tasks (rate method):</div>
              {plannedData.filter(t=>(t.cc_qty||t.volume)&&t.cc_dur>0).slice(0,6).map((t,i) => {
                const vol = t.cc_qty||t.volume||0;
                const rate = t.cc_dur > 0 ? (vol/t.cc_dur).toFixed(1) : '—';
                return (
                  <div key={i} style={{ color:C.text, padding:'2px 0', borderBottom:`1px solid ${C.border}` }}>
                    {t.activity.slice(0,40)} | {vol.toLocaleString()} m³ ÷ {t.cc_dur}d = <span style={{color:'#6366f1',fontWeight:700}}>{rate} m³/d</span> | {t.cc_s}→{t.cc_e}
                  </div>
                );
              })}
            </div>
      ...
      {allKeys.length > 0 ? (
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:18, marginBottom:16 }}>
          <div style={{ position:'relative', height: groupBy==='month' ? 260 : 220 }}>
            <canvas ref={chartRef}/>
          </div>
        </div>
      ) : (
        <div style={{ background:C.panel, border:`1px dashed ${C.border}`, borderRadius:12, padding:'40px 20px', marginBottom:16, textAlign:'center', color:C.muted }}>
          <div style={{ fontSize:32, marginBottom:8 }}>📊</div>
          <div style={{ fontSize:13, fontWeight:700, color:C.sub }}>No overlapping data yet</div>
          <div style={{ fontSize:11, marginTop:4 }}>Load both the Schedule Excel and the Logistics Excel to see the comparison</div>
        </div>
      )}
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden', marginBottom:16 }}>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg }}>
                  {['Period','Planned Excav. (m³)','KTC Actual (m³)','Delta (m³)','Achievement','Tasks'].map(h => (
                    <th key={h} style={{ padding:'8px 14px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:h==='Period'?'left':'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {allKeys.map((k, i) => {
                  const p    = Math.round(plannedByBucket[k]?.planned||0);
                  const a    = Math.round(actualByBucket[k]||0);
                  const d    = a - p;
                  const pct  = p > 0 ? ((a/p)*100).toFixed(0) : null;
                  const pctN = pct ? parseFloat(pct) : null;
                  const dc   = d >= 0 ? '#22c55e' : '#ef4444';
                  const ac   = pctN===null?C.muted:pctN>=80?'#22c55e':pctN>=50?'#f59e0b':'#ef4444';
                  const nT   = plannedByBucket[k]?.tasks?.length || 0;
                  const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                  return (
                    <tr key={k} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                      onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                      onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                      <td style={{ padding:'8px 14px', fontWeight:700, color:C.text, fontFamily:'monospace' }}>{labelOf(k)}</td>
                      <td style={{ padding:'8px 14px', textAlign:'center', color:'#6366f1', fontWeight:700, fontFamily:'monospace' }}>{p>0?p.toLocaleString():'—'}</td>
                      <td style={{ padding:'8px 14px', textAlign:'center', color:'#fb923c', fontWeight:700, fontFamily:'monospace' }}>{a>0?a.toLocaleString():'—'}</td>
                      <td style={{ padding:'8px 14px', textAlign:'center', fontWeight:800, fontFamily:'monospace', color:p>0||a>0?dc:C.muted }}>
                        {p>0||a>0?`${d>=0?'+':''}${d.toLocaleString()}`:'—'}
                      </td>
                      <td style={{ padding:'8px 14px', textAlign:'center' }}>
                        {pct!==null ? <span style={{ fontSize:12, fontWeight:900, color:ac }}>{pct}%</span>
                          : a>0 ? <span style={{ fontSize:9, color:C.muted }}>unplanned</span>
                          : <span style={{ fontSize:9, color:C.muted }}>—</span>}
                      </td>
                      <td style={{ padding:'8px 14px', textAlign:'center', color:C.sub, fontSize:10 }}>{nT||'—'}</td>
                    </tr>
                  );
                })}
                <tr style={{ background:C.tableHead, borderTop:`2px solid ${C.muted}` }}>
                  <td style={{ padding:'8px 14px', fontWeight:900, color:C.text }}>TOTAL</td>
                  <td style={{ padding:'8px 14px', textAlign:'center', color:'#6366f1', fontWeight:900, fontFamily:'monospace' }}>{totalPlanned>0?Math.round(totalPlanned).toLocaleString():'—'}</td>
                  <td style={{ padding:'8px 14px', textAlign:'center', color:'#fb923c', fontWeight:900, fontFamily:'monospace' }}>{totalActual>0?Math.round(totalActual).toLocaleString():'—'}</td>
                  <td style={{ padding:'8px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:totalActual>=totalPlanned?'#22c55e':'#ef4444' }}>
                    {totalPlanned>0||totalActual>0?`${Math.round(totalActual-totalPlanned)>=0?'+':''}${Math.round(totalActual-totalPlanned).toLocaleString()}`:'—'}
                  </td>
                  <td style={{ padding:'8px 14px', textAlign:'center' }}>
                    {achievement&&<span style={{ fontSize:13, fontWeight:900, color:achCol }}>{achievement}%</span>}
                  </td>
                  <td style={{ padding:'8px 14px', textAlign:'center', color:C.sub }}>{plannedData.length||'—'}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {/* ── ZONE BREAKDOWN ── */}
      {showZones && plannedData.length > 0 && (
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden' }}>
          <div style={{ padding:'10px 16px', borderBottom:`1px solid ${C.border}`, fontSize:11, fontWeight:700, color:C.text }}>
            Planned Excavation by Task — {plannedData.length} tasks · sorted by volume
          </div>
          <div style={{ overflowX:'auto', maxHeight:400, overflowY:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg, position:'sticky', top:0 }}>
                  {['Zone','Activity','Level','CC Start','CC End','cc_qty','volume'].map(h => (
                    <th key={h} style={{ padding:'7px 12px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:h==='Activity'?'left':'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {plannedData.sort((a,b)=>(b.cc_qty||b.volume||0)-(a.cc_qty||a.volume||0)).map((t,i) => {
                  const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                  return (
                    <tr key={t.id||i} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                      onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                      onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:'#6366f1', fontWeight:700, fontSize:10 }}>{t.zone}</td>
                      <td style={{ padding:'5px 12px', color:C.text, fontSize:10, maxWidth:260, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={t.activity}>{t.activity}</td>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:C.sub, fontSize:10 }}>{t.level||'—'}</td>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:C.sub, fontSize:10, fontFamily:'monospace' }}>{t.cc_s||'—'}</td>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:'#6366f1', fontSize:10, fontFamily:'monospace' }}>{t.cc_e||'—'}</td>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:(t.cc_qty||0)>0?'#22c55e':C.muted, fontWeight:700, fontFamily:'monospace' }}>{(t.cc_qty||0)>0?t.cc_qty.toLocaleString():'—'}</td>
                      <td style={{ padding:'5px 12px', textAlign:'center', color:(t.volume||0)>0?'#6366f1':C.muted, fontFamily:'monospace' }}>{(t.volume||0)>0?t.volume.toLocaleString():'—'}</td>
                    </tr>
                  );
                })}
              </tbody>
...
// ─── LOGISTICS VIEW ───────────────────────────────────────────────────────────
function LogisticsView({ C, lightMode, rows, setRows, files, setFiles, screen, setScreen }) {
  const [parsing,      setParsing]      = useState(false);
  const [parseMsg,     setParseMsg]     = useState('');
  ...
  const [filterSDC,    setFilterSDC]    = useState('all');
  const [searchQ,      setSearchQ]      = useState('');
  const [sortCol,      setSortCol]      = useState('date');
  const [sortDir,      setSortDir]      = useState('asc');
  const [page,         setPage]         = useState(1);
  const PAGE_SIZE = 50;
  const [ktcGroupBy,   setKtcGroupBy]   = useState('week');
  const [ktcMatFilter, setKtcMatFilter] = useState('all');
  const [volGroupBy,   setVolGroupBy]   = useState('month'); // for the volume comparison chart

  const fileInputRef3  = useRef(null);
  const ktcChartRef    = useRef(null);
  const ktcChartInst   = useRef(null);
  const volChartRef    = useRef(null);
  const volChartInst   = useRef(null);
  const [xlsxReady2, setXlsxReady2] = useState(!!window.XLSX);
  const [chartJsReady2, setChartJsReady2] = useState(!!window.Chart);
  useEffect(() => {
    if (window.XLSX) { setXlsxReady2(true); return; }
    const s = document.createElement('script');
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js';
    s.onload = () => setXlsxReady2(true);
    document.head.appendChild(s);
  }, []);
  useEffect(() => {
    if (window.Chart) { setChartJsReady2(true); return; }
    const s = document.createElement('script');
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js';
    s.onload = () => setChartJsReady2(true);
    document.head.appendChild(s);
  }, []);

  // ── KTC Tipper volume aggregation (always uses full rows, ignores filters) ──
  const ktcChartData = useMemo(() => {
    const KTC_VOL = 8;
    // All KTC tipper rows across full dataset
    let ktcRows = rows.filter(r => r.subcon.includes('KTC') && r.vtype.includes('TIPPER'));
    if (!ktcRows.length) return null;

    // Material filter
    if (ktcMatFilter === 'disposal') ktcRows = ktcRows.filter(r => r.mat.includes('DISPOSAL'));
    else if (ktcMatFilter === 'filling') ktcRows = ktcRows.filter(r => r.mat.includes('FILLING') || r.mat === 'SOIL');
    ...
    ktcRows.forEach(r => {
      let key;
      if (ktcGroupBy === 'day')   key = r.date;
      else if (ktcGroupBy === 'week')  key = weekKey(r.date);
      else                         key = r.month;
      if (!buckets[key]) buckets[key] = { trips:0, vol:0 };
      buckets[key].trips += (r.trips || 0);
      buckets[key].vol   += (r.trips || 0) * KTC_VOL;
    });

    const sorted = Object.entries(buckets).sort(([a],[b]) => a.localeCompare(b));
    return {
      labels: sorted.map(([k]) => {
        if (ktcGroupBy === 'month') return new Date(k+'-15').toLocaleDateString('en-SG',{month:'short',year:'2-digit'});
        if (ktcGroupBy === 'week')  return k.replace('-W',' W');
        return k.slice(5); // day: MM-DD
      }),
      trips:  sorted.map(([,v]) => v.trips),
      vols:   sorted.map(([,v]) => v.vol),
      keys:   sorted.map(([k]) => k),
      totalTrips: ktcRows.reduce((s,r) => s+(r.trips||0), 0),
      totalVol:   ktcRows.reduce((s,r) => s+(r.trips||0)*KTC_VOL, 0),
      days: new Set(ktcRows.map(r=>r.date)).size,
    };
  }, [rows, ktcGroupBy, ktcMatFilter]);

  // ── Volume comparison chart: Tipper (×8) + Concrete (×6) by month ──
  const TIPPER_M3 = 8, CONCRETE_M3 = 6;
  const volChartData = useMemo(() => {
    if (!rows.length) return null;
    const buckets = {}; // month -> { tipper, concrete, tipperTrips, concreteTrips }
    rows.forEach(r => {
      const k = r.month;
      if (!k) return;
      if (!buckets[k]) buckets[k] = { tipper:0, concrete:0, tipperTrips:0, concreteTrips:0 };
      if (r.vtype.includes('TIPPER'))   { buckets[k].tipper        += (r.trips||0)*TIPPER_M3;   buckets[k].tipperTrips   += (r.trips||0); }
      if (r.vtype.includes('CONCRETE')) { buckets[k].concrete      += (r.trips||0)*CONCRETE_M3; buckets[k].concreteTrips += (r.trips||0); }
    });
    const sorted = Object.entries(buckets).sort(([a],[b])=>a.localeCompare(b));
    const lbl = k => { const d=new Date(k+'-15'); return isNaN(d)?k:d.toLocaleDateString('en-SG',{month:'short',year:'2-digit'}); };
    const totalTipperVol    = rows.filter(r=>r.vtype.includes('TIPPER'))  .reduce((s,r)=>s+(r.trips||0)*TIPPER_M3,0);
    const totalConcreteVol  = rows.filter(r=>r.vtype.includes('CONCRETE')).reduce((s,r)=>s+(r.trips||0)*CONCRETE_M3,0);
    const totalTipperTrips  = rows.filter(r=>r.vtype.includes('TIPPER'))  .reduce((s,r)=>s+(r.trips||0),0);
    const totalConcreteTrips= rows.filter(r=>r.vtype.includes('CONCRETE')).reduce((s,r)=>s+(r.trips||0),0);
    return { labels:sorted.map(([k])=>lbl(k)), tipper:sorted.map(([,v])=>Math.round(v.tipper)), concrete:sorted.map(([,v])=>Math.round(v.concrete)),
             tipperTrips:sorted.map(([,v])=>v.tipperTrips), concreteTrips:sorted.map(([,v])=>v.concreteTrips),
             months:sorted.map(([k])=>k), totalTipperVol, totalConcreteVol, totalTipperTrips, totalConcreteTrips };
  }, [rows]);

  // ── Tipper material breakdown by month ──
  const tipperMatData = useMemo(() => {
    const cats = { 'SOIL FILLING':0, 'SOIL DISPOSAL':0, 'HARDCORE DISPOSAL':0, 'HARDCORE':0, 'SAND FILLING':0, OTHER:0 };
    rows.filter(r=>r.vtype.includes('TIPPER')).forEach(r => {
      const m = r.mat.trim();
      if (cats[m] !== undefined) cats[m] += (r.trips||0)*TIPPER_M3;
      else cats.OTHER += (r.trips||0)*TIPPER_M3;
    });
    return Object.entries(cats).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  }, [rows]);

  // ── Concrete material breakdown ──
  const concreteMatData = useMemo(() => {
    const cats = { 'CONCRETE':0, 'LSS':0, OTHER:0 };
    rows.filter(r=>r.vtype.includes('CONCRETE')).forEach(r => {
      const m = r.mat.trim() || 'CONCRETE';
      if (cats[m] !== undefined) cats[m] += (r.trips||0)*CONCRETE_M3;
      else cats.OTHER += (r.trips||0)*CONCRETE_M3;
    });
    return Object.entries(cats).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  }, [rows]);

  // ── Monthly detail table for volume tracker ──
  const volMonthlyTable = useMemo(() => {
    if (!volChartData) return [];
    return volChartData.months.map((mo, i) => ({
      label: volChartData.labels[i],
      tipperTrips: volChartData.tipperTrips[i],
      concreteTrips: volChartData.concreteTrips[i],
      tipperVol: volChartData.tipper[i],
      concreteVol: volChartData.concrete[i],
      totalVol: volChartData.tipper[i] + volChartData.concrete[i],
    }));
  }, [volChartData]);

  // ── Build volume comparison chart ──
  function buildVolCompChart(lm) {
    if (!window.Chart || !volChartData || !volChartRef.current) return;
    if (volChartInst.current) volChartInst.current.destroy();
    const gridCol = lm ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.05)';
    const tickCol = lm ? '#64748b' : '#6b7280';
    volChartInst.current = new window.Chart(volChartRef.current, {
      type: 'bar',
      data: {
        labels: volChartData.labels,
        datasets: [
          { label:'Tipper Truck (×8 m³)', data:volChartData.tipper,    backgroundColor:'rgba(251,146,60,0.5)', borderColor:'#fb923c', borderWidth:1, borderRadius:4, borderSkipped:false },
          { label:'Concrete Truck (×6 m³)',data:volChartData.concrete,  backgroundColor:'rgba(59,130,246,0.45)',borderColor:'#3b82f6', borderWidth:1, borderRadius:4, borderSkipped:false },
        ],
      },
      options: {
        responsive:true, maintainAspectRatio:false,
        ...
        plugins:{
          ...
          tooltip:{
            callbacks:{
              label: ctx => `${ctx.dataset.label.split('(')[0].trim()}: ${ctx.raw.toLocaleString()} m³`,
              afterBody: items => {
                const ti = items.find(i=>i.datasetIndex===0); const ci = items.find(i=>i.datasetIndex===1);
                const total = (ti?.raw||0)+(ci?.raw||0);
                const idx = ti?.dataIndex ?? 0;
                return [
                  `Tipper trips: ${volChartData.tipperTrips[idx]}`,
                  `Concrete trips: ${volChartData.concreteTrips[idx]}`,
                  `Combined: ${total.toLocaleString()} m³`,
                ];
              },
            },
          },
        ...
        scales:{
          x:{ grid:{display:false}, ticks:{color:tickCol,font:{size:10},maxRotation:45} },
          y:{ stacked:false, grid:{color:gridCol}, ticks:{color:tickCol,font:{size:10},callback:n=>n>=1000?`${(n/1000).toFixed(0)}k`:n},
            title:{display:true,text:'Volume (m³)',color:C.sub,font:{size:9,weight:'700'}}, beginAtZero:true },
        },
      },
    });
  ...
  useEffect(() => {
    if (!chartJsReady2 || !volChartData) return;
    if (volChartRef.current) { buildVolCompChart(lightMode); return; }
    const raf = requestAnimationFrame(() => {
      if (volChartRef.current) buildVolCompChart(lightMode);
      else setTimeout(() => buildVolCompChart(lightMode), 80);
    });
    return () => cancelAnimationFrame(raf);
  }, [volChartData, chartJsReady2, lightMode, screen]);

  // ── Build KTC Chart.js bar chart ──
  function buildKtcChart(data, lm, matFilter) {
    if (!window.Chart || !data) return;
    const canvas = ktcChartRef.current;
    if (!canvas) return;
    if (ktcChartInst.current) ktcChartInst.current.destroy();
    const gridCol  = lm ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.05)';
    const tickCol  = lm ? '#64748b' : '#6b7280';
    const peak     = data.vols.indexOf(Math.max(...data.vols));
    const baseCol  = matFilter === 'disposal' ? '#ef4444' : matFilter === 'filling' ? '#10b981' : '#fb923c';
    const baseDim  = matFilter === 'disposal' ? 'rgba(239,68,68,0.3)' : matFilter === 'filling' ? 'rgba(16,185,129,0.3)' : 'rgba(251,146,60,0.35)';
    ktcChartInst.current = new window.Chart(canvas, {
      type: 'bar',
      data: {
        labels: data.labels,
        datasets: [
          {
            label: 'Volume (m³)',
            data: data.vols,
            backgroundColor: data.vols.map((_,i) => i===peak ? baseCol : baseDim),
            borderColor:     data.vols.map((_,i) => i===peak ? baseCol : baseDim.replace('0.3','0.6')),
          ...
          },
          {
            ...
            borderWidth: 2,
            pointRadius: data.labels.length > 20 ? 2 : 4,
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              label: ctx => ctx.datasetIndex === 0
                ? `Volume: ${Math.round(ctx.raw).toLocaleString()} m³`
                : `Trips: ${ctx.raw}`,
            },
          },
        },
        scales: {
          x: {
            grid: { display: false },
            ticks: { color: tickCol, font: { size: 10 }, maxRotation: 45, autoSkip: data.labels.length > 20 },
          },
          yVol: {
            position: 'left',
            grid: { color: gridCol },
            ticks: { color: '#fb923c', font: { size: 10 }, callback: n => n >= 1000 ? `${(n/1000).toFixed(1)}k` : n },
            title: { display: true, text: 'Volume (m³)', color: '#fb923c', font: { size: 9, weight: '700' } },
            beginAtZero: true,
          },
          yTrips: {
            position: 'right',
            ...
            title: { display: true, text: 'Trips', color: '#f59e0b', font: { size: 9, weight: '700' } },
            beginAtZero: true,
          },
        },
      },
  ...
  useEffect(() => {
    if (!chartJsReady2 || !ktcChartData) return;
    if (ktcChartRef.current) {
      buildKtcChart(ktcChartData, lightMode, ktcMatFilter);
    } else {
      const raf = requestAnimationFrame(() => {
        if (ktcChartRef.current) {
          buildKtcChart(ktcChartData, lightMode, ktcMatFilter);
        } else {
          const t = setTimeout(() => buildKtcChart(ktcChartData, lightMode, ktcMatFilter), 80);
          return () => clearTimeout(t);
        }
      });
      return () => cancelAnimationFrame(raf);
    }
  }, [ktcChartData, chartJsReady2, lightMode, screen, ktcMatFilter]);
  ...
  async function handleParse() {
    ...
    setParsing(true); setParsePct(0); setParseMsg('');
    const allRows = [];
    for (let fi = 0; fi < files.length; fi++) {
      setParseMsg(`Parsing ${fi+1}/${files.length}: ${files[fi].name}`);
      setParsePct(Math.round((fi / files.length) * 80));
      try {
        const buf = await files[fi].arrayBuffer();
        const wb  = window.XLSX.read(buf, { type:'array', cellDates:false, raw:true });
        // Parse all sheets whose name matches DAILY LIST pattern
        for (const sheetName of wb.SheetNames) {
          if (!/DAILY LIST/i.test(sheetName)) continue;
          const ws   = wb.Sheets[sheetName];
          const data = window.XLSX.utils.sheet_to_json(ws, { header:1, defval:'', raw:true });
          // Find header row (contains "DELIVERY DATE")
          let hdrIdx = -1;
          for (let r = 0; r < Math.min(5, data.length); r++) {
            if (data[r].some(c => String(c).toUpperCase().includes('DELIVERY DATE'))) { hdrIdx = r; break; }
          }
          if (hdrIdx < 0) continue;
          // Map columns by header label
          const hdr = data[hdrIdx].map(h => String(h).trim().toUpperCase().replace(/\s+/g,' ').replace(/\n/g,' '));
          const ci = (label) => hdr.findIndex(h => h.includes(label));
          const iDate  = ci('DELIVERY DATE');
          const iTiming= ci('TIMING');
          const iTrips = ci('TOTAL TRIP');
          const iSDC   = ci('SDC ESCORT');
          const iSubcon= ci('SUBCON');
          const iVType = ci('VEHICLE TYPE');
          const iDim   = ci('DIMENSION');
          const iTruck = ci('TRUCK DETAIL');
          const iDriver= ci('DRIVER NAME');
          const iLoc   = ci('DELIVERY LOCATION');
          const iMat   = ci('PURPOSE') >= 0 ? ci('PURPOSE') : ci('MATERIALS');
          const iPIC   = ci('CJY PIC');
          if (iDate < 0) continue;
          for (let r = hdrIdx + 1; r < data.length; r++) {
            const row = data[r];
            const rawDate = row[iDate] !== undefined ? row[iDate] : '';
            const dateStr = serialToDate(rawDate);
            if (!dateStr) continue; // skip non-data rows
            const trips = parseFloat(row[iTiming >= 0 && String(row[iTiming]).match(/^\d+$/) ? iTiming : iTrips]) || 1;
            // iTrips col — col index 3 is Total Trip in this sheet layout
            const totalTrips = parseFloat(iTrips >= 0 ? row[iTrips] : 1) || 1;
            allRows.push({
              date:   dateStr,
              month:  dateStr.slice(0,7),
              timing: iTiming >= 0 ? String(row[iTiming]||'').trim() : '—',
              trips:  totalTrips,
              sdc:    iSDC   >= 0 ? String(row[iSDC]  ||'').trim().toUpperCase() : '—',
              subcon: iSubcon>= 0 ? String(row[iSubcon]||'').trim().toUpperCase() : '—',
              vtype:  iVType >= 0 ? String(row[iVType] ||'').trim().toUpperCase() : '—',
              dim:    iDim   >= 0 ? String(row[iDim]   ||'').trim() : '—',
              truck:  iTruck >= 0 ? String(row[iTruck] ||'').trim().toUpperCase() : '—',
              driver: iDriver>= 0 ? String(row[iDriver]||'').trim() : '—',
              loc:    iLoc   >= 0 ? String(row[iLoc]   ||'').trim().toUpperCase() : '—',
              mat:    iMat   >= 0 ? String(row[iMat]   ||'').trim().toUpperCase() : '—',
              pic:    iPIC   >= 0 ? String(row[iPIC]   ||'').trim() : '—',
              sheet:  sheetName,
            });
          }
        }
      } catch(e) { console.warn('Parse error', files[fi].name, e); }
    }
    setParsePct(100);
    setParseMsg(`✓ ${allRows.length.toLocaleString()} delivery records from ${files.length} file${files.length>1?'s':''}`);
    setRows(allRows);
    setParsing(false);
    setPage(1);
    setTimeout(() => setScreen('dashboard'), 300);
  ...
  const vtypes    = useMemo(() => [...new Set(rows.map(r => r.vtype).filter(Boolean))].sort(), [rows]);
  const materials = useMemo(() => [...new Set(rows.map(r => r.mat).filter(Boolean))].sort(), [rows]);

  const filtered = useMemo(() => {
    let r = rows;
    if (activeMonth !== 'all') r = r.filter(x => x.month === activeMonth);
    if (filterVType  !== 'all') r = r.filter(x => x.vtype === filterVType);
    if (filterMat    !== 'all') r = r.filter(x => x.mat   === filterMat);
    if (filterSDC    !== 'all') r = r.filter(x => x.sdc   === filterSDC);
    if (searchQ.trim()) {
      const q = searchQ.trim().toLowerCase();
      r = r.filter(x => x.subcon.toLowerCase().includes(q) || x.mat.toLowerCase().includes(q) || x.truck.toLowerCase().includes(q) || x.driver.toLowerCase().includes(q) || x.vtype.toLowerCase().includes(q));
    }
    return r.sort((a,b) => {
      let va = a[sortCol] ?? '', vb = b[sortCol] ?? '';
      if (sortCol === 'trips') { va = parseFloat(va)||0; vb = parseFloat(vb)||0; }
      if (va < vb) return sortDir === 'asc' ? -1 : 1;
      if (va > vb) return sortDir === 'asc' ? 1 : -1;
      return 0;
    });
  }, [rows, activeMonth, filterVType, filterMat, filterSDC, searchQ, sortCol, sortDir]);

  // KPIs
  const totalEntries  = filtered.length;
  const totalTrips    = filtered.reduce((s,r) => s + (r.trips||0), 0);
  const sdcCount      = filtered.filter(r => r.sdc === 'YES').length;
  const sdcPct        = totalEntries > 0 ? ((sdcCount/totalEntries)*100).toFixed(0) : 0;
  const uniqueDates   = new Set(filtered.map(r => r.date)).size;
  const avgTripsDay   = uniqueDates > 0 ? (totalTrips/uniqueDates).toFixed(1) : '—';

  // Material breakdown
  const matBreakdown = useMemo(() => {
    const m = {};
    filtered.forEach(r => { m[r.mat] = (m[r.mat]||0) + (r.trips||1); });
    return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,8);
  }, [filtered]);

  // Vehicle type breakdown
  const vtBreakdown = useMemo(() => {
    const m = {};
    filtered.forEach(r => { m[r.vtype] = (m[r.vtype]||0) + 1; });
    return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,6);
  }, [filtered]);

  // Subcon breakdown
  const subconBreakdown = useMemo(() => {
    const m = {};
    filtered.forEach(r => {
      const s = r.subcon.replace(/\s*\/\s*/g,'/').replace(/\s+/g,' ').trim();
      m[s] = (m[s]||0) + (r.trips||1);
    });
    return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,8);
  }, [filtered]);
  ...
  const MAT_COLORS = ['#3b82f6','#10b981','#f59e0b','#8b5cf6','#ef4444','#06b6d4','#f97316','#22c55e'];
  const VT_COLORS  = ['#6366f1','#fb923c','#0891b2','#16a34a','#dc2626','#7c3aed'];

  // ── mini bar chart (SVG) ──
  function MiniBar({ data, colors, unit='trips', labelWidth=160 }) {
    const max = Math.max(...data.map(([,v])=>v), 1);
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
        {data.map(([label, val], i) => (
          <div key={i} style={{ display:'grid', gridTemplateColumns:`${labelWidth}px 1fr 50px`, alignItems:'center', gap:8 }}>
            <div style={{ fontSize:10, color:C.sub, textAlign:'right', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={label}>{label || '—'}</div>
            <div style={{ background:C.inputBg, borderRadius:3, height:18, overflow:'hidden' }}>
              <div style={{ width:`${(val/max)*100}%`, height:'100%', background:colors[i%colors.length], borderRadius:3, transition:'width 0.4s', opacity:0.85 }}/>
            </div>
            <div style={{ fontSize:10, fontWeight:700, color:C.text, fontFamily:'monospace', textAlign:'right' }}>{val.toLocaleString()}</div>
          </div>
        ))}
      </div>
    );
  ...
  const SortTh = ({ col, label, align='center' }) => {
    ...
    return (
      <th onClick={() => toggleSort(col)} style={{ padding:'8px 10px', color:active?C.amber:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:align, borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap', cursor:'pointer', userSelect:'none', background:active?C.amber+'11':undefined }}>
  ...
  const VTYPE_COLOR = {
    "40'FT FLATBED":'#6366f1',"40'FT LOWBED":'#8b5cf6',"40'FT CONTAINER":'#a78bfa',
    'TIPPER TRUCK':'#fb923c','CONCRETE TRUCK':'#3b82f6','DIESEL TRUCK':'#f59e0b',
    'LORRY CRANE':'#10b981','SMALL LORRY':'#06b6d4','WASTE TRUCK':'#ef4444',
    'CONCRETE PUMP':'#22c55e',
  };
  const MAT_COLOR = {
    'REBAR CAGE':'#6366f1','REBAR':'#8b5cf6','CONCRETE':'#3b82f6','DIESEL':'#f59e0b',
    'SOIL FILLING':'#fb923c','SOIL DISPOSAL':'#f97316','PRECAST':'#10b981',
    'DECK PANEL':'#06b6d4','SITE MATERIAL':'#94a3b8','SUPPLY MATERIALS':'#64748b',
    'EMPTY TRAILER':'#475569','DEMOB':'#ef4444',
  };
      ...
      <div style={{ background:C.bg, minHeight:'60vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', fontFamily:"'Poppins',sans-serif" }}>
        <div style={{ width:'100%', maxWidth:600 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:28 }}>
            ...
            <div>
              <div style={{ fontSize:18, fontWeight:900, color:C.text }}>Logistics <span style={{ color:'#3b82f6' }}>Delivery Plan</span></div>
              <div style={{ fontSize:11, color:C.sub, marginTop:2, fontFamily:'monospace' }}>RWS P1 · CJY Daily Traffic Delivery Plan · Upload Excel</div>
            </div>
          </div>

          <div style={{ border:`1.5px dashed #3b82f666`, borderRadius:14, padding:'2rem 1.5rem', textAlign:'center', cursor:'pointer', background:C.panel, position:'relative', marginBottom:14 }}
            onClick={() => fileInputRef3.current?.click()}
            onDragOver={e => e.preventDefault()}
            onDrop={e => { e.preventDefault(); addFiles([...e.dataTransfer.files]); }}>
            <input ref={fileInputRef3} type="file" multiple accept=".xlsx,.xls" style={{ display:'none' }}
              onChange={e => addFiles([...e.target.files])} />
            <div style={{ fontSize:36, marginBottom:10 }}>📂</div>
            <div style={{ fontSize:15, fontWeight:700, color:C.text, marginBottom:4 }}>Drag & drop the Delivery Plan Excel</div>
            <div style={{ fontSize:11, color:C.sub, fontFamily:'monospace' }}>DAILY MATERIALS DELIVERY PLAN · .xlsx · all months parsed automatically</div>
          </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:14 }}>
              {files.map((f,i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:6, background:C.panel, border:`1px solid ${C.border}`, borderRadius:8, padding:'5px 12px', fontSize:11, fontFamily:'monospace' }}>
                  ...
                  <span onClick={() => setFiles(prev => prev.filter((_,idx)=>idx!==i))} style={{ cursor:'pointer', color:C.sub, marginLeft:4, fontSize:14 }}>×</span>
                </div>
              ))}
            </div>
          )}

          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 16px', marginBottom:12, fontSize:11, color:C.sub, fontFamily:'monospace' }}>
            💡 All <strong style={{color:C.text}}>DAILY LIST</strong> sheets are parsed automatically. Hidden sheets are included.
            Columns detected: Delivery Date · Timing · Total Trip · SDC Escort · Subcon · Vehicle Type · Materials Type
          </div>

          {parsing && (
            <div style={{ marginBottom:10 }}>
              <div style={{ height:3, background:C.inputBg, borderRadius:2, overflow:'hidden', marginBottom:6 }}>
                <div style={{ height:'100%', background:'#3b82f6', borderRadius:2, transition:'width 0.3s', width:`${parsePct}%` }}/>
              </div>
          ...
          <button onClick={handleParse} disabled={!files.length || parsing || !xlsxReady2}
            style={{ width:'100%', padding:'13px', background:'#3b82f6', color:'#fff', border:'none', borderRadius:12, fontSize:14, fontWeight:800, fontFamily:'inherit', cursor:files.length&&!parsing?'pointer':'not-allowed', opacity:files.length&&!parsing?1:0.4 }}>
            {parsing ? 'Parsing…' : files.length > 0 ? `Parse ${files.length} file${files.length>1?'s':''} →` : 'Select file to continue →'}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ background:C.bg, minHeight:'60vh', padding:'20px 24px', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
            <span style={{ background:'#3b82f6', color:'#fff', fontSize:10, fontWeight:800, padding:'3px 10px', borderRadius:4, letterSpacing:1, textTransform:'uppercase' }}>Logistics</span>
            <span style={{ color:C.sub, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>Delivery Plan Analytics</span>
          </div>
          <div style={{ fontSize:18, fontWeight:900, color:C.text }}>RWS P1 — Daily Traffic Delivery Records</div>
          <div style={{ fontSize:11, color:C.sub, marginTop:3, fontFamily:'monospace' }}>
            {rows.length.toLocaleString()} records · {months.length} month{months.length!==1?'s':''}
          </div>
          {/* File chips */}
          <div style={{ display:'flex', gap:6, marginTop:5, flexWrap:'wrap' }}>
            {files.map((f,i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:5, background:'#3b82f618', border:`1px solid #3b82f644`, borderRadius:6, padding:'2px 9px' }}>
                <span style={{ width:5, height:5, borderRadius:'50%', background:'#3b82f6', flexShrink:0 }}/>
                <span style={{ fontSize:10, color:'#93c5fd', fontFamily:'monospace' }}>{f.name}</span>
              </div>
            ))}
          </div>
        </div>
        <button onClick={() => { setScreen('upload'); setRows([]); setFiles([]); setParseMsg(''); }}
          style={{ fontSize:11, color:C.sub, cursor:'pointer', fontFamily:'inherit', border:`1px solid ${C.border}`, borderRadius:8, padding:'7px 14px', background:'transparent' }}>
          ← upload more
        </button>
      </div>
      ...
      <div style={{ display:'flex', background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, marginBottom:16, overflow:'hidden', flexWrap:'wrap' }}>
        {[
          ['Total Entries',    totalEntries.toLocaleString(),            '#3b82f6'],
          ['Total Trips',      Math.round(totalTrips).toLocaleString(),  '#f59e0b'],
          ['Days Covered',     uniqueDates,                               C.text  ],
          ['Avg Trips / Day',  avgTripsDay,                              '#10b981'],
          ['SDC Escorted',     `${sdcCount} (${sdcPct}%)`,              '#8b5cf6'],
        ].map(([l,v,c],i) => (
          <div key={i} style={{ flex:1, minWidth:120, padding:'10px 16px', borderRight:`1px solid ${C.border}` }}>
            <div style={{ fontSize:9, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.5 }}>{l}</div>
            <div style={{ fontSize:16, fontWeight:900, color:c, marginTop:2, fontFamily:'monospace' }}>{v}</div>
      ...
      <div style={{ display:'flex', gap:6, marginBottom:16, flexWrap:'wrap' }}>
        {['all', ...months].map(m => (
          <button key={m} onClick={() => { setActiveMonth(m); setPage(1); }}
            style={{ padding:'4px 14px', borderRadius:20, border:`1px solid ${activeMonth===m?'#3b82f6':C.border}`, background:activeMonth===m?'#3b82f622':'transparent', color:activeMonth===m?'#3b82f6':C.sub, cursor:'pointer', fontSize:10, fontWeight:700, fontFamily:'inherit' }}>
            {m === 'all' ? 'All Months' : new Date(m+'-15').toLocaleDateString('en-SG',{month:'short',year:'numeric'})}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap', alignItems:'center' }}>
        {/* Search */}
        <input value={searchQ} onChange={e=>{setSearchQ(e.target.value);setPage(1);}} placeholder="Search subcon, material, truck…"
          style={{ padding:'6px 12px', borderRadius:8, border:`1px solid ${C.border}`, background:C.inputBg, color:C.text, fontSize:11, fontFamily:'inherit', outline:'none', minWidth:220 }}/>
        {/* Vehicle type */}
        <select value={filterVType} onChange={e=>{setFilterVType(e.target.value);setPage(1);}}
          style={{ padding:'6px 10px', borderRadius:8, border:`1px solid ${C.border}`, background:C.inputBg, color:C.sub, fontSize:11, fontFamily:'inherit', outline:'none', cursor:'pointer' }}>
          <option value="all">All Vehicle Types</option>
          {vtypes.map(v => <option key={v} value={v}>{v}</option>)}
        </select>
        {/* Material */}
        <select value={filterMat} onChange={e=>{setFilterMat(e.target.value);setPage(1);}}
          style={{ padding:'6px 10px', borderRadius:8, border:`1px solid ${C.border}`, background:C.inputBg, color:C.sub, fontSize:11, fontFamily:'inherit', outline:'none', cursor:'pointer' }}>
        ...
        {/* SDC */}
        {['all','YES','NO'].map(s => (
          <button key={s} onClick={()=>{setFilterSDC(s);setPage(1);}}
            style={{ padding:'4px 12px', borderRadius:20, border:`1px solid ${filterSDC===s?'#8b5cf6':C.border}`, background:filterSDC===s?'#8b5cf622':'transparent', color:filterSDC===s?'#8b5cf6':C.sub, cursor:'pointer', fontSize:10, fontWeight:700, fontFamily:'inherit' }}>
            {s==='all'?'All':s==='YES'?'✓ SDC Escort':'✕ No Escort'}
          </button>
        ))}
        <span style={{ marginLeft:'auto', fontSize:10, color:C.muted, fontStyle:'italic' }}>{filtered.length.toLocaleString()} entries</span>
      </div>

      {/* ── KTC SOIL MOVEMENT CHART ── */}
      {ktcChartData && (
        <div style={{ background:C.panel, border:`1px solid #fb923c44`, borderLeft:`3px solid #fb923c`, borderRadius:12, padding:18, marginBottom:16 }}>
          {/* header row */}
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10, marginBottom:14 }}>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                <span style={{ fontSize:9, fontWeight:800, color:'#fb923c', background:'#fb923c18', padding:'2px 8px', borderRadius:4, textTransform:'uppercase', letterSpacing:0.5 }}>KTC Tipper</span>
                <span style={{ fontSize:12, fontWeight:800, color:C.text }}>Soil Movement — Volume & Trip Count</span>
              </div>
              <div style={{ fontSize:10, color:C.sub, fontFamily:'monospace' }}>
                {ktcMatFilter === 'all' ? 'All soil types' : ktcMatFilter === 'disposal' ? 'Soil Disposal only' : 'Soil Filling only'}
                &nbsp;· 8 m³/trip · {ktcChartData.days} active days ·&nbsp;
                <span style={{ color:'#fb923c', fontWeight:700 }}>{Math.round(ktcChartData.totalVol).toLocaleString()} m³</span>
                &nbsp;total ·&nbsp;
                <span style={{ color:'#f59e0b', fontWeight:700 }}>{ktcChartData.totalTrips.toLocaleString()} trips</span>
              </div>
            </div>
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              {/* Material filter */}
              <div style={{ display:'flex', gap:3, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
                {[['all','All Soil'],['disposal','Disposal'],['filling','Filling']].map(([k,l]) => {
                  const col = k==='disposal'?'#ef4444':k==='filling'?'#10b981':'#fb923c';
                  return (
                    <button key={k} onClick={() => setKtcMatFilter(k)} style={{
                      padding:'4px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:700,
                      fontFamily:'inherit', transition:'all 0.15s',
                      background: ktcMatFilter===k ? col : 'transparent',
                      color:      ktcMatFilter===k ? '#fff' : C.sub,
                    }}>{l}</button>
                  );
                })}
              </div>
              ...
              <div style={{ display:'flex', gap:3, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
                {[['day','Daily'],['week','Weekly'],['month','Monthly']].map(([k,l]) => (
                  <button key={k} onClick={() => setKtcGroupBy(k)} style={{
                    padding:'4px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:10, fontWeight:700,
                    fontFamily:'inherit', transition:'all 0.15s',
                    background: ktcGroupBy===k ? '#fb923c' : 'transparent',
                    color:      ktcGroupBy===k ? '#000'    : C.sub,
                  }}>{l}</button>
                ))}
              </div>
            </div>
          ...
          {/* chart */}
          <div style={{ position:'relative', height: ktcGroupBy==='day' ? 260 : 220 }}>
            <canvas ref={ktcChartRef}/>
          </div>
          {/* peak callout */}
          {ktcChartData.vols.length > 0 && (() => {
            const peakIdx = ktcChartData.vols.indexOf(Math.max(...ktcChartData.vols));
            const peakVol = ktcChartData.vols[peakIdx];
            const peakTrips = ktcChartData.trips[peakIdx];
            const peakLabel = ktcChartData.labels[peakIdx];
            const avgVol = Math.round(ktcChartData.totalVol / ktcChartData.vols.length);
            return (
              <div style={{ display:'flex', gap:10, marginTop:12, flexWrap:'wrap' }}>
                {[
                  [`Peak ${ktcGroupBy==='day'?'Day':ktcGroupBy==='week'?'Week':'Month'}`, `${peakLabel}`, '#fb923c'],
                  ['Peak Volume',  `${Math.round(peakVol).toLocaleString()} m³`, '#fb923c'],
                  ['Peak Trips',   peakTrips, '#f59e0b'],
                  [`Avg / ${ktcGroupBy==='day'?'Day':ktcGroupBy==='week'?'Week':'Month'}`, `${avgVol.toLocaleString()} m³`, C.sub],
                ].map(([l,v,col]) => (
                  <div key={l} style={{ background:C.bg, borderRadius:8, padding:'7px 14px', border:`1px solid ${C.border}`, flex:1, minWidth:100 }}>
                    <div style={{ fontSize:8, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.5, marginBottom:2 }}>{l}</div>
      ...
      {/* ── VOLUME COMPARISON CHART: TIPPER vs CONCRETE ── */}
      {volChartData && (
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:18, marginBottom:16 }}>
          {/* Header */}
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:10, marginBottom:14 }}>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                <span style={{ fontSize:9, fontWeight:800, color:'#fb923c', background:'#fb923c18', padding:'2px 8px', borderRadius:4, textTransform:'uppercase', letterSpacing:0.5 }}>Tipper</span>
                <span style={{ fontSize:9, fontWeight:800, color:'#3b82f6', background:'#3b82f618', padding:'2px 8px', borderRadius:4, textTransform:'uppercase', letterSpacing:0.5 }}>Concrete</span>
                <span style={{ fontSize:12, fontWeight:800, color:C.text }}>Volume Analysis — Monthly</span>
              </div>
              <div style={{ fontSize:10, color:C.sub, fontFamily:'monospace' }}>
                Tipper × {TIPPER_M3} m³ &nbsp;·&nbsp; Concrete × {CONCRETE_M3} m³ &nbsp;·&nbsp; all vehicles (not just KTC)
              </div>
            </div>
            {/* KPI mini strip */}
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {[
                [`${Math.round(volChartData.totalTipperVol).toLocaleString()} m³`, `Tipper (${volChartData.totalTipperTrips.toLocaleString()} trips)`, '#fb923c'],
                [`${Math.round(volChartData.totalConcreteVol).toLocaleString()} m³`, `Concrete (${volChartData.totalConcreteTrips.toLocaleString()} trips)`, '#3b82f6'],
                [`${Math.round(volChartData.totalTipperVol+volChartData.totalConcreteVol).toLocaleString()} m³`, 'Combined total', '#10b981'],
              ].map(([v,l,c]) => (
                <div key={l} style={{ background:C.bg, borderRadius:8, padding:'7px 14px', border:`1px solid ${C.border}`, minWidth:110, textAlign:'center' }}>
                  <div style={{ fontSize:8, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.4, marginBottom:2 }}>{l}</div>
          ...
          {/* Chart */}
          <div style={{ position:'relative', height:240 }}><canvas ref={volChartRef}/></div>
        </div>
      )}

      {/* ── MATERIAL BREAKDOWN TABLES (Tipper + Concrete side by side) ── */}
      {(tipperMatData.length > 0 || concreteMatData.length > 0) && (
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:16 }}>
          {/* Tipper material breakdown */}
          <div style={{ background:C.panel, border:`1px solid #fb923c33`, borderLeft:`3px solid #fb923c`, borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'10px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div style={{ fontSize:10, fontWeight:800, textTransform:'uppercase', color:'#fb923c', letterSpacing:0.5 }}>🚛 Tipper — Material Breakdown</div>
              <div style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>× {TIPPER_M3} m³/trip</div>
            </div>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg }}>
                  {['Material','Trips','Volume (m³)','Share'].map(h=>(
                    <th key={h} style={{ padding:'6px 12px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:h==='Material'?'left':'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(() => {
                  const totalVol = tipperMatData.reduce((s,[,v])=>s+v,0);
                  return tipperMatData.map(([mat,vol],i) => {
                    const trips = Math.round(vol/TIPPER_M3);
                    const pct = totalVol>0?((vol/totalVol)*100).toFixed(1):0;
                    const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                    const matCol = mat.includes('FILLING')?'#fb923c':mat.includes('DISPOSAL')||mat.includes('HARDCORE')?'#f97316':mat.includes('SAND')?'#f59e0b':'#94a3b8';
                    return (
                      <tr key={mat} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                        onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                        onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                        <td style={{ padding:'6px 12px' }}>
                          <span style={{ fontSize:9, fontWeight:700, padding:'1px 6px', borderRadius:10, background:matCol+'22', color:matCol, border:`1px solid ${matCol}44` }}>{mat||'OTHER'}</span>
                        </td>
                        <td style={{ padding:'6px 12px', textAlign:'center', color:C.text, fontFamily:'monospace', fontWeight:700 }}>{trips.toLocaleString()}</td>
                        <td style={{ padding:'6px 12px', textAlign:'center', color:'#fb923c', fontFamily:'monospace', fontWeight:800 }}>{Math.round(vol).toLocaleString()}</td>
                        <td style={{ padding:'6px 12px', textAlign:'center' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <div style={{ flex:1, height:4, background:C.inputBg, borderRadius:2, overflow:'hidden' }}>
                              <div style={{ width:`${pct}%`, height:'100%', background:'#fb923c', borderRadius:2 }}/>
                            </div>
                            <span style={{ fontSize:9, color:C.sub, minWidth:30, textAlign:'right' }}>{pct}%</span>
                          </div>
                        </td>
                      </tr>
                    );
                ...
                <tr style={{ background:C.tableHead, borderTop:`2px solid ${C.muted}` }}>
                  <td style={{ padding:'6px 12px', fontWeight:900, color:C.text, fontSize:10 }}>TOTAL</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:C.text }}>{volChartData?.totalTipperTrips.toLocaleString()}</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:'#fb923c' }}>{Math.round(volChartData?.totalTipperVol||0).toLocaleString()}</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontSize:9, color:C.sub }}>100%</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          {/* Concrete material breakdown */}
          <div style={{ background:C.panel, border:`1px solid #3b82f633`, borderLeft:`3px solid #3b82f6`, borderRadius:12, overflow:'hidden' }}>
            <div style={{ padding:'10px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div style={{ fontSize:10, fontWeight:800, textTransform:'uppercase', color:'#3b82f6', letterSpacing:0.5 }}>🪣 Concrete Truck — Material Breakdown</div>
              <div style={{ fontSize:9, color:C.sub, fontFamily:'monospace' }}>× {CONCRETE_M3} m³/trip</div>
            </div>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg }}>
              ...
              <tbody>
                {(() => {
                  const totalVol = concreteMatData.reduce((s,[,v])=>s+v,0);
                  return concreteMatData.map(([mat,vol],i) => {
                    const trips = Math.round(vol/CONCRETE_M3);
                    const pct = totalVol>0?((vol/totalVol)*100).toFixed(1):0;
                    const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                    const matCol = mat==='CONCRETE'?'#3b82f6':mat==='LSS'?'#6366f1':'#94a3b8';
                    return (
                      <tr key={mat} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                        onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                        onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                        <td style={{ padding:'6px 12px' }}>
                          <span style={{ fontSize:9, fontWeight:700, padding:'1px 6px', borderRadius:10, background:matCol+'22', color:matCol, border:`1px solid ${matCol}44` }}>{mat||'CONCRETE'}</span>
                        </td>
                        <td style={{ padding:'6px 12px', textAlign:'center', color:C.text, fontFamily:'monospace', fontWeight:700 }}>{trips.toLocaleString()}</td>
                        <td style={{ padding:'6px 12px', textAlign:'center', color:'#3b82f6', fontFamily:'monospace', fontWeight:800 }}>{Math.round(vol).toLocaleString()}</td>
                        <td style={{ padding:'6px 12px', textAlign:'center' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <div style={{ flex:1, height:4, background:C.inputBg, borderRadius:2, overflow:'hidden' }}>
                ...
                <tr style={{ background:C.tableHead, borderTop:`2px solid ${C.muted}` }}>
                  <td style={{ padding:'6px 12px', fontWeight:900, color:C.text, fontSize:10 }}>TOTAL</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:C.text }}>{volChartData?.totalConcreteTrips.toLocaleString()}</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:'#3b82f6' }}>{Math.round(volChartData?.totalConcreteVol||0).toLocaleString()}</td>
                  <td style={{ padding:'6px 12px', textAlign:'center', fontSize:9, color:C.sub }}>100%</td>
                </tr>
              </tbody>
            </table>
      ...
      {volMonthlyTable.length > 0 && (
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', marginBottom:16 }}>
          <div style={{ padding:'10px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <span style={{ fontSize:11, fontWeight:800, color:C.text }}>Monthly Volume Summary</span>
            <span style={{ fontSize:10, color:C.sub, fontFamily:'monospace' }}>Tipper × {TIPPER_M3} m³ · Concrete × {CONCRETE_M3} m³</span>
          </div>
          <div style={{ overflowX:'auto', maxHeight:360, overflowY:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
              <thead>
                <tr style={{ background:C.bg, position:'sticky', top:0, zIndex:3 }}>
                  {['Month','Tipper Trips','Tipper Vol (m³)','Concrete Trips','Concrete Vol (m³)','Combined Vol (m³)'].map(h=>(
                    <th key={h} style={{ padding:'7px 14px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:h==='Month'?'left':'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {volMonthlyTable.map((row,i) => {
                  const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                  const maxVol = Math.max(...volMonthlyTable.map(r=>r.totalVol),1);
                  return (
                    <tr key={row.label} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                      onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                      onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                      <td style={{ padding:'7px 14px', fontWeight:700, color:C.text, fontFamily:'monospace' }}>{row.label}</td>
                      <td style={{ padding:'7px 14px', textAlign:'center', color:C.text, fontFamily:'monospace' }}>{row.tipperTrips.toLocaleString()}</td>
                      <td style={{ padding:'7px 14px', textAlign:'center', color:'#fb923c', fontWeight:800, fontFamily:'monospace' }}>{row.tipperVol.toLocaleString()}</td>
                      <td style={{ padding:'7px 14px', textAlign:'center', color:C.text, fontFamily:'monospace' }}>{row.concreteTrips.toLocaleString()}</td>
                      <td style={{ padding:'7px 14px', textAlign:'center', color:'#3b82f6', fontWeight:800, fontFamily:'monospace' }}>{row.concreteVol.toLocaleString()}</td>
                      <td style={{ padding:'7px 14px', textAlign:'center' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                          <div style={{ flex:1, height:5, background:C.inputBg, borderRadius:2, overflow:'hidden' }}>
                            <div style={{ width:`${(row.totalVol/maxVol)*100}%`, height:'100%', background:'#10b981', borderRadius:2 }}/>
                          </div>
                          <span style={{ fontWeight:800, color:'#10b981', fontFamily:'monospace', minWidth:60, textAlign:'right' }}>{row.totalVol.toLocaleString()}</span>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                <tr style={{ background:C.tableHead, borderTop:`2px solid ${C.muted}` }}>
                  <td style={{ padding:'7px 14px', fontWeight:900, color:C.text }}>TOTAL</td>
                  <td style={{ padding:'7px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:C.text }}>{volChartData?.totalTipperTrips.toLocaleString()}</td>
                  <td style={{ padding:'7px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:'#fb923c' }}>{Math.round(volChartData?.totalTipperVol||0).toLocaleString()}</td>
                  <td style={{ padding:'7px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:C.text }}>{volChartData?.totalConcreteTrips.toLocaleString()}</td>
                  <td style={{ padding:'7px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:'#3b82f6' }}>{Math.round(volChartData?.totalConcreteVol||0).toLocaleString()}</td>
                  <td style={{ padding:'7px 14px', textAlign:'center', fontWeight:900, fontFamily:'monospace', color:'#10b981' }}>{Math.round((volChartData?.totalTipperVol||0)+(volChartData?.totalConcreteVol||0)).toLocaleString()}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14, marginBottom:16 }}>
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:16 }}>
          <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:C.sub, marginBottom:12, fontFamily:'monospace' }}>Material Type (trips)</div>
          <MiniBar data={matBreakdown} colors={MAT_COLORS} labelWidth={140}/>
        </div>
        {/* Vehicle */}
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:16 }}>
          <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:C.sub, marginBottom:12, fontFamily:'monospace' }}>Vehicle Type (entries)</div>
          <MiniBar data={vtBreakdown} colors={VT_COLORS} labelWidth={140}/>
        </div>
        {/* Subcon */}
        <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:12, padding:16 }}>
          <div style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:C.sub, marginBottom:12, fontFamily:'monospace' }}>Subcontractor (trips)</div>
          <MiniBar data={subconBreakdown} colors={MAT_COLORS.slice().reverse()} labelWidth={140}/>
        </div>
      </div>
      ...
      <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, overflow:'hidden', marginBottom:12 }}>
        <div style={{ overflowX:'auto', maxHeight:460, overflowY:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
            <thead>
              <tr style={{ background:C.bg, position:'sticky', top:0, zIndex:5 }}>
                <SortTh col="date"   label="Date"     align="center"/>
                <SortTh col="timing" label="Timing"   align="center"/>
                <SortTh col="trips"  label="Trips"    align="center"/>
                <th style={{ padding:'8px 10px', color:'#fb923c', fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>Vol m³ <span style={{fontSize:7,opacity:0.7}}>(T×8 / C×6)</span></th>
                <th style={{ padding:'8px 10px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>SDC</th>
                <SortTh col="subcon" label="Subcon"   align="left"/>
                <SortTh col="vtype"  label="Vehicle"  align="left"/>
                <SortTh col="mat"    label="Material" align="left"/>
                <SortTh col="truck"  label="Truck"    align="center"/>
                <SortTh col="driver" label="Driver"   align="left"/>
                <SortTh col="pic"    label="CJY PIC"  align="left"/>
              </tr>
            </thead>
            <tbody>
              {pageRows.map((r,i) => {
                const vtCol  = VTYPE_COLOR[r.vtype] || '#94a3b8';
                const matCol = MAT_COLOR[r.mat]  || '#64748b';
                const rowBg  = i%2===0 ? C.tableRow0 : C.tableRow1;
                return (
                  <tr key={i} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                    onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                    onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                    <td style={{ padding:'6px 10px', textAlign:'center', color:C.text, fontWeight:700, fontSize:10, fontFamily:'monospace', whiteSpace:'nowrap' }}>{r.date}</td>
                    <td style={{ padding:'6px 10px', textAlign:'center', color:C.sub, fontSize:9, whiteSpace:'nowrap' }}>{r.timing}</td>
                    <td style={{ padding:'6px 10px', textAlign:'center', fontWeight:800, fontFamily:'monospace', fontSize:11, color:'#f59e0b' }}>{r.trips}</td>
                    <td style={{ padding:'6px 10px', textAlign:'center', fontFamily:'monospace', fontSize:11 }}>
                      {r.vtype.includes('TIPPER')
                        ? <span style={{ color:'#fb923c', fontWeight:800 }}>{(r.trips * 8).toLocaleString()}</span>
                        : r.vtype.includes('CONCRETE')
                        ? <span style={{ color:'#3b82f6', fontWeight:800 }}>{(r.trips * 6).toLocaleString()}</span>
                        : <span style={{ color:C.muted, fontSize:9 }}>—</span>}
                    </td>
                    <td style={{ padding:'6px 10px', textAlign:'center' }}>
                      <span style={{ fontSize:8, fontWeight:700, padding:'2px 6px', borderRadius:10, background:r.sdc==='YES'?'#8b5cf622':'transparent', color:r.sdc==='YES'?'#8b5cf6':C.muted, border:`1px solid ${r.sdc==='YES'?'#8b5cf644':C.border}` }}>{r.sdc}</span>
                    </td>
                    <td style={{ padding:'6px 10px', color:C.text, fontSize:10, whiteSpace:'nowrap' }}>{r.subcon}</td>
                    <td style={{ padding:'6px 10px' }}>
                      <span style={{ fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:20, background:vtCol+'22', color:vtCol, border:`1px solid ${vtCol}44`, whiteSpace:'nowrap' }}>{r.vtype}</span>
                    </td>
                    <td style={{ padding:'6px 10px' }}>
                      <span style={{ fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:20, background:matCol+'22', color:matCol, border:`1px solid ${matCol}44`, whiteSpace:'nowrap' }}>{r.mat || '—'}</span>
                    </td>
                    <td style={{ padding:'6px 10px', textAlign:'center', color:C.sub, fontSize:10, fontFamily:'monospace' }}>{r.truck}</td>
                    <td style={{ padding:'6px 10px', color:C.sub, fontSize:10 }}>{r.driver}</td>
                    <td style={{ padding:'6px 10px', color:C.sub, fontSize:10 }}>{r.pic}</td>
                  </tr>
                );
              })}
              {pageRows.length === 0 && (
                <tr><td colSpan={11} style={{ padding:'32px', textAlign:'center', color:C.muted, fontStyle:'italic' }}>No entries match this filter</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* ── PAGINATION ── */}
      {totalPages > 1 && (
        <div style={{ display:'flex', gap:6, alignItems:'center', justifyContent:'flex-end', fontSize:11, fontFamily:'monospace', flexWrap:'wrap' }}>
          <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
            style={{ padding:'4px 10px', border:`1px solid ${C.border}`, borderRadius:6, background:'transparent', color:page===1?C.muted:C.sub, cursor:page===1?'not-allowed':'pointer' }}>‹ Prev</button>
          {Array.from({length:Math.min(7,totalPages)},(_,i)=>{
            const pg = totalPages <= 7 ? i+1 : page <= 4 ? i+1 : page >= totalPages-3 ? totalPages-6+i : page-3+i;
            return (
              <button key={pg} onClick={()=>setPage(pg)}
                style={{ padding:'4px 10px', border:`1px solid ${page===pg?'#3b82f6':C.border}`, borderRadius:6, background:page===pg?'#3b82f622':'transparent', color:page===pg?'#3b82f6':C.sub, cursor:'pointer', fontWeight:page===pg?700:400 }}>{pg}</button>
            );
          })}
          <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
            style={{ padding:'4px 10px', border:`1px solid ${C.border}`, borderRadius:6, background:'transparent', color:page===totalPages?C.muted:C.sub, cursor:page===totalPages?'not-allowed':'pointer' }}>Next ›</button>
          <span style={{ color:C.muted }}>Page {page}/{totalPages} · {filtered.length.toLocaleString()} entries</span>
        </div>
      )}
    </div>
  );
...
function TipperTruckView({ C, lightMode }) {
  ...
  const [volM3,         setVolM3]         = useState(8);
  ...
  const [volDash,       setVolDash]       = useState(8);

  // ── KTC planned state ──
  const [logFile,       setLogFile]       = useState(null);   // single Delivery Plan file
  const [logParsing,    setLogParsing]    = useState(false);
  const [logMsg,        setLogMsg]        = useState('');
  const [ktcPlanned,    setKtcPlanned]    = useState({});     // { 'YYYY-MM-DD': totalTrips }
  ...
  // ── Parse Delivery Plan Excel for KTC Tipper planned trips ──
  async function parseLogisticsForKTC(file) {
    if (!file || !window.XLSX) return;
    setLogParsing(true); setLogMsg('Reading delivery plan…');
    try {
      const buf = await file.arrayBuffer();
      const wb  = window.XLSX.read(buf, { type:'array', cellDates:false, raw:true });
      const byDate = {};
      ...
      for (const sheetName of wb.SheetNames) {
        ...
        for (let r = hdrIdx + 1; r < data.length; r++) {
          const row    = data[r];
          const dateStr= serialToDate(row[1]);
          if (!dateStr) continue;
          const subcon = String(row[5]||'').trim().toUpperCase();
          const vtype  = String(row[6]||'').trim().toUpperCase();
          const trips  = parseFloat(row[3]) || 1;
          // Match KTC (any variant) + TIPPER TRUCK
          const isKTC    = subcon.includes('KTC');
          const isTipper = vtype.includes('TIPPER');
          if (isKTC && isTipper) {
            byDate[dateStr] = (byDate[dateStr] || 0) + trips;
            totalFound++;
          }
        }
      }
      setKtcPlanned(byDate);
      setLogMsg(`✓ ${totalFound} KTC tipper entries across ${Object.keys(byDate).length} days`);
    } catch(e) {
      setLogMsg('⚠ Could not parse delivery plan');
    }
    setLogParsing(false);
  }
  ...
  // ── Persistent hidden input for KTC logistics file (works on both screens) ──
  const logFileInput = (
    <input ref={logFileRef} type="file" accept=".xlsx,.xls" style={{display:'none'}}
      onChange={e => { const f = e.target.files[0]; if(f){setLogFile(f);parseLogisticsForKTC(f);} }}/>
  );
      ...
      <div style={{ background:C.bg, minHeight:'60vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', fontFamily:"'Poppins',sans-serif" }}>
        ...
        <div style={{ width:'100%', maxWidth:580 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 16px', marginBottom:12 }}>
            ...
            <input id="tt-vol-input" type="number" defaultValue={8} min={1} max={30} step={0.5}
            ...
            <span style={{ fontSize:10, color:C.muted, fontFamily:'monospace', marginLeft:'auto' }}>KTC tipper: 8 m³</span>
          </div>

          {/* KTC Planned upload (optional) */}
          <div style={{ border:`1px solid #fb923c44`, borderRadius:12, padding:'14px 16px', background:C.panel, marginBottom:12 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
              <span style={{ fontSize:9, fontWeight:800, color:'#fb923c', background:'#fb923c18', padding:'2px 8px', borderRadius:4, textTransform:'uppercase', letterSpacing:0.5 }}>Optional</span>
              <span style={{ fontSize:11, fontWeight:700, color:C.text }}>KTC Planned Soil Movement</span>
            </div>
            <div style={{ fontSize:10, color:C.sub, fontFamily:'monospace', marginBottom:10 }}>
              Upload the <strong style={{color:C.text}}>Daily Materials Delivery Plan</strong> Excel to overlay planned KTC tipper trips vs actual SmartSite trips.
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
              <button onClick={() => logFileRef.current?.click()}
                style={{ padding:'7px 14px', borderRadius:8, border:`1px solid ${logFile?'#fb923c':C.border}`, background:logFile?'#fb923c22':C.inputBg, color:logFile?'#fb923c':C.sub, fontSize:11, fontFamily:'inherit', fontWeight:700, cursor:'pointer' }}>
                {logFile ? `✓ ${logFile.name.slice(0,30)}…` : '📋 Upload Delivery Plan Excel'}
              </button>
              {logFile && <button onClick={()=>{setLogFile(null);setKtcPlanned({});setLogMsg('');}} style={{fontSize:11,color:C.sub,border:'none',background:'transparent',cursor:'pointer'}}>✕ remove</button>}
            </div>
            {logMsg && <div style={{ fontSize:10, color: logMsg.startsWith('✓')?'#10b981':'#f59e0b', fontFamily:'monospace', marginTop:8 }}>{logParsing?'⏳ ':''}{logMsg}</div>}
          </div>
    ...
    <div style={{ background:C.bg, minHeight:'60vh', padding:'20px 24px', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          ...
          <button onClick={()=>logFileRef.current?.click()}
            style={{ fontSize:11, cursor:'pointer', fontFamily:'inherit', border:`1px solid ${logFile?'#fb923c':C.border}`, borderRadius:8, padding:'7px 14px', background:logFile?'#fb923c18':'transparent', color:logFile?'#fb923c':C.sub, fontWeight:700, transition:'all 0.15s', whiteSpace:'nowrap' }}>
            {logFile ? `📋 KTC: ${Object.keys(ktcPlanned).length}d` : '📋 + KTC Plan'}
          </button>
        </div>
      </div>
      ...
      {/* ── KTC PLANNED SOIL MOVEMENT ── */}
      {Object.keys(ktcPlanned).length > 0 && (() => {
        const allDates = [...new Set([...sortedDays, ...Object.keys(ktcPlanned)])].sort();
        const rows = allDates.map(d => {
          const planned = ktcPlanned[d] || 0;
          const dayIdx  = sortedDays.indexOf(d);
          const actual  = dayIdx >= 0 ? (dayTrips[dayIdx] || 0) : 0;
          const planVol = Math.round(planned * volDash);
          const actVol  = Math.round(actual  * volDash);
          const delta   = actual - planned;
          const deltaVol= actVol - planVol;
          return { d, planned, actual, planVol, actVol, delta, deltaVol };
        });
        const totPlan   = rows.reduce((s,r)=>s+r.planned,0);
        const totAct    = rows.reduce((s,r)=>s+r.actual,0);
        const totPlanV  = Math.round(totPlan * volDash);
        const totActV   = Math.round(totAct  * volDash);
        const coverage  = totPlan > 0 ? ((totAct/totPlan)*100).toFixed(1) : '—';
        return (
          <div style={{ background:C.panel, border:`1px solid #fb923c44`, borderLeft:`3px solid #fb923c`, borderRadius:12, padding:18, marginBottom:14 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:12, marginBottom:14 }}>
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                  <span style={{ fontSize:9, fontWeight:800, color:'#fb923c', background:'#fb923c18', padding:'2px 8px', borderRadius:4, textTransform:'uppercase', letterSpacing:0.5 }}>KTC Tipper</span>
                  <span style={{ fontSize:12, fontWeight:800, color:C.text }}>Planned vs Actual Soil Movement</span>
                </div>
                <div style={{ fontSize:10, color:C.sub, fontFamily:'monospace' }}>
                  From Delivery Plan · Filtered: KTC subcon + TIPPER TRUCK · {volDash} m³/trip
                </div>
              </div>
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                {[
                  ['Planned Trips', totPlan, '#fb923c'],
                  ['Actual Trips',  totAct,  '#f59e0b'],
                  ['Planned Vol',   `${totPlanV.toLocaleString()} m³`, '#fb923c'],
                  ['Actual Vol',    `${totActV.toLocaleString()} m³`,  '#10b981'],
                  ['Achievement',   `${coverage}%`, parseFloat(coverage)>=90?'#22c55e':parseFloat(coverage)>=60?'#f59e0b':'#ef4444'],
                ].map(([l,v,c]) => (
                  <div key={l} style={{ background:C.bg, borderRadius:8, padding:'8px 14px', border:`1px solid ${C.border}`, minWidth:90, textAlign:'center' }}>
                    <div style={{ fontSize:8, color:C.sub, textTransform:'uppercase', fontWeight:700, letterSpacing:0.5, marginBottom:2 }}>{l}</div>
            ...
            <div style={{ overflowX:'auto', maxHeight:340, overflowY:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
                <thead>
                  <tr style={{ background:C.bg, position:'sticky', top:0, zIndex:3 }}>
                    {['Date','Day','Planned Trips','Actual Trips','Δ Trips','Planned Vol (m³)','Actual Vol (m³)','Δ Vol (m³)','Achievement'].map(h => (
                      <th key={h} style={{ padding:'7px 10px', color:C.sub, fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {rows.map((r,i) => {
                    const ach = r.planned > 0 ? ((r.actual/r.planned)*100).toFixed(0) : r.actual > 0 ? '—' : '—';
                    const achNum = parseFloat(ach);
                    const achCol = isNaN(achNum) ? C.muted : achNum>=90?'#22c55e':achNum>=60?'#f59e0b':'#ef4444';
                    const deltaCol = r.delta > 0 ? '#22c55e' : r.delta < 0 ? '#ef4444' : C.sub;
                    const rowBg = i%2===0?C.tableRow0:C.tableRow1;
                    const dow = new Date(r.d).toLocaleDateString('en-SG',{weekday:'short'});
                    return (
                      <tr key={r.d} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                        onMouseEnter={e=>e.currentTarget.style.background=C.tableHover}
                        onMouseLeave={e=>e.currentTarget.style.background=rowBg}>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', fontSize:10, color:C.text, fontWeight:700, whiteSpace:'nowrap' }}>{r.d}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center' }}><span style={{ fontSize:8, fontWeight:700, padding:'1px 6px', borderRadius:4, background:'#fb923c18', color:'#fb923c' }}>{dow}</span></td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:'#fb923c', fontWeight:700 }}>{r.planned||'—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:'#f59e0b', fontWeight:700 }}>{r.actual||'—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:deltaCol, fontWeight:800 }}>{r.planned||r.actual ? (r.delta>0?'+':'')+r.delta : '—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:C.sub }}>{r.planVol||'—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:'#10b981', fontWeight:700 }}>{r.actVol||'—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center', fontFamily:'monospace', color:deltaCol, fontWeight:700 }}>{r.planned||r.actual ? (r.deltaVol>0?'+':'')+r.deltaVol : '—'}</td>
                        <td style={{ padding:'5px 10px', textAlign:'center' }}>
                          {r.planned > 0
                            ? <span style={{ fontSize:10, fontWeight:800, color:achCol }}>{ach}%</span>
                            : <span style={{ fontSize:9, color:C.muted }}>unplanned</span>}
                        </td>
                      </tr>
                    );
                  })}
