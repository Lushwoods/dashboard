    ...
    <div className="dash-root" style={{fontFamily:"'Poppins',sans-serif",background:C.bg,minHeight:"100vh",width:"100vw",maxWidth:"100vw",color:C.text,fontSize:13,overflowX:"hidden"}}>
      ...
      <div style={{background:C.nav,borderBottom:`1px solid ${C.border}`,padding:"12px 20px 0 20px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",paddingBottom:8,borderBottom:`1px solid ${C.border}`}}>
          {tab !== "trucks" && tab !== "logistics" && <><span style={{color:C.sub,fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginRight:4}}>Area:</span>
          {AREA_GROUPS.map(ag=>(
        ...
        {tab!=="analysis" && tab!=="trucks" && tab!=="logistics" && <div style={{paddingTop:7,paddingBottom:7,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"flex-start",gap:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",paddingTop:7,paddingBottom:10}}>
          {!zoneDef?.levelFilter && tab!=="analysis" && tab!=="trucks" && tab!=="logistics" && (() => {
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
            {[["gantt","📊 Gantt",false,null],["analysis","📈 Productivity",false,null],["cards","🃏 Prod Cards",false,null],["resources","📋 Resources",true,null],["scurve","📉 S-Curve",true,null],["settlement","🏦 Settlement",true,null],["health","🏥 Schedule Health",true,null],["hedging","⚖️ Hedging",true,null],["trucks","🚛 Excavation",true,"SmartSite"],["logistics","📦 Logistics",false,null]].map(([v,l,playground,badge])=>(
      ...
      {tasks.length === 0 && tab !== "trucks" && tab !== "logistics" && (
...
// ─── LOGISTICS VIEW ───────────────────────────────────────────────────────────
function LogisticsView({ C, lightMode }) {
  ...
  const PAGE_SIZE = 50;
  const [ktcGroupBy,   setKtcGroupBy]   = useState('week'); // 'day' | 'week' | 'month'
  ...
  const ktcChartData = useMemo(() => {
    ...
    const ktcRows = rows.filter(r => r.subcon.includes('KTC') && r.vtype.includes('TIPPER'));
  ...
  function buildKtcChart(data, lm) {
    if (!window.Chart || !data) return;
    const canvas = ktcChartRef.current;
    if (!canvas) return; // ref not mounted yet — caller retries via rAF
        ...
        datasets: [
          {
            label: 'Volume (m³)',
            data: data.vols,
            backgroundColor: data.vols.map((_,i) => i===peak ? '#fb923c' : 'rgba(251,146,60,0.35)'),
            borderColor:     data.vols.map((_,i) => i===peak ? '#fb923c' : 'rgba(251,146,60,0.6)'),
  ...
  useEffect(() => {
    ...
    // Try immediately; if canvas not yet in DOM (screen just flipped to dashboard),
    // retry on next animation frame then once more after a short delay
    if (ktcChartRef.current) {
      buildKtcChart(ktcChartData, lightMode);
    } else {
      const raf = requestAnimationFrame(() => {
        if (ktcChartRef.current) {
          buildKtcChart(ktcChartData, lightMode);
        } else {
          const t = setTimeout(() => buildKtcChart(ktcChartData, lightMode), 80);
          return () => clearTimeout(t);
        }
      });
      return () => cancelAnimationFrame(raf);
    }
  }, [ktcChartData, chartJsReady2, lightMode, screen]);
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:11, color:C.sub, marginTop:2, fontFamily:'monospace' }}>
            {rows.length.toLocaleString()} total records · {months.length} month{months.length!==1?'s':''} · {files.length} file{files.length!==1?'s':''}
          </div>
        </div>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10, marginBottom:14 }}>
            <div>
              ...
              <div style={{ fontSize:10, color:C.sub, fontFamily:'monospace' }}>
                All KTC subcon variants · 8 m³/trip · {ktcChartData.days} active days ·&nbsp;
            ...
            {/* Group-by toggle */}
            <div style={{ display:'flex', gap:4, background:C.inputBg, borderRadius:8, padding:3, border:`1px solid ${C.border}` }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
            <thead>
              <tr style={{ background:C.bg, position:'sticky', top:0, zIndex:5 }}>
                ...
                <th style={{ padding:'8px 10px', color:'#fb923c', fontSize:9, textTransform:'uppercase', fontWeight:700, textAlign:'center', borderBottom:`1px solid ${C.border}`, whiteSpace:'nowrap' }}>Vol m³ <span style={{fontSize:7,opacity:0.7}}>(KTC·8)</span></th>
                ...
                return (
                  <tr key={i} style={{ background:rowBg, borderBottom:`1px solid ${C.border}` }}
                    ...
                    <td style={{ padding:'6px 10px', textAlign:'center', fontFamily:'monospace', fontSize:11 }}>
                      {r.subcon.includes('KTC') && r.vtype.includes('TIPPER')
