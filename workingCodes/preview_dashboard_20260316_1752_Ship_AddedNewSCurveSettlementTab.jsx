...
export default function App() {
  ...
  return (
    <div className="dash-root" style={{fontFamily:"'Poppins',sans-serif",background:C.bg,minHeight:"100vh",width:"100vw",maxWidth:"100vw",color:C.text,fontSize:13,overflowX:"hidden"}}>
      ...
      {tasks.length > 0 && tab==="settlement" && <SettlementView tasks={tasks} col={col} C={C}/>}
...
// ─── SETTLEMENT ───────────────────────────────────────────────────────────────
const SETTLEMENT_QUARTERS = [
  { key:'q1', label:'Q1', period:'03.19 – 06.18 2026', start:'2026-03-19', end:'2026-06-18', rec:'+15d', delay:'-93d', prog:15 },
  { key:'q2', label:'Q2', period:'06.19 – 09.18 2026', start:'2026-06-19', end:'2026-09-18', rec:'+20d', delay:'-73d', prog:35 },
  { key:'q3', label:'Q3', period:'09.19 – 12.18 2026', start:'2026-09-19', end:'2026-12-18', rec:'+30d', delay:'-43d', prog:65 },
  { key:'q4', label:'Q4', period:'12.19 2026 – 05.19 2027', start:'2026-12-19', end:'2027-05-19', rec:'+43d', delay:'0d', prog:100 },
];
...
function SettlementView({ tasks, col, C }) {
  ...
  // Per-task settlement calculation using time-ratio model
  const settled = useMemo(() => {
    if (!qDef) return [];
    return tasks.filter(t => t.cc_s && t.cc_e).map(t => {
      ...
      return { ...t, overlapDur: Math.round(overlapDur), totalDur: Math.round(totalDur),
               qQty, cumDone, cumPct, allocPct, qty, type, active: overlapDur > 0 };
    });
  }, [tasks, activeQ]);

  // Category aggregates
  const catTotals = useMemo(() => {
    const agg = { demolish:{total:0,done:0}, excavate:{total:0,done:0}, slab:{total:0,done:0}, column:{total:0,done:0} };
    settled.forEach(t => {
      if (agg[t.type]) { agg[t.type].total += t.qty; agg[t.type].done += t.cumDone; }
    });
    return agg;
  }, [settled]);
  ...
  const zoneTotals = (zTasks) => ({
    qty: zTasks.reduce((s,t) => s+t.qty, 0),
    done: zTasks.reduce((s,t) => s+t.cumDone, 0),
  });

  return (
    <div style={{ background:C.panel, minHeight:'100vh', fontFamily:"'Poppins',sans-serif" }}>

      {/* ── Header ── */}
      <div style={{ background:C.panel, padding:'20px 24px', color:C.text }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:16 }}>
          <div>
            <div style={{ fontSize:14, fontWeight:900, textTransform:'uppercase', letterSpacing:1, textDecoration:'underline', textDecorationColor:'#3b82f6' }}>
              M19 – M33 Settlement Board · 15-20-30-43 Scheme
            </div>
            <div style={{ fontSize:9, color:'#94a3b8', marginTop:4, letterSpacing:1, textTransform:'uppercase' }}>
              19-Day Milestone Cycle · Time-Ratio Settlement · Target: 108d Hedge Recovery
            </div>
          </div>
          {/* Hedging bar */}
          <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:8, padding:'10px 16px', minWidth:320 }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:9, fontWeight:700, color:'#60a5fa', marginBottom:6 }}>
              <span>Annual Hedge Trajectory (−108d → 0d)</span>
              <span style={{ color:C.text, fontFamily:'monospace' }}>{qDef?.delay} / 0 Days</span>
            </div>
            <div style={{ height:8, background:C.border, borderRadius:99, overflow:'hidden' }}>
              <div style={{ width:`${qDef?.prog||0}%`, height:'100%', background:'#3b82f6', borderRadius:99, transition:'width 0.6s ease' }}/>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:7, color:C.muted, marginTop:6, fontFamily:'monospace' }}>
              <span>Start (-108)</span><span>Q1 (-93)</span><span>Q2 (-73)</span><span>Q3 (-43)</span>
              <span style={{ color:'#34d399', fontWeight:800 }}>Q4 (0d)</span>
            </div>
          </div>
          {/* Quarter tabs */}
          <div style={{ display:'flex', background:C.tableHead, borderRadius:8, padding:4, border:`1px solid ${C.border}`, gap:2 }}>
            {SETTLEMENT_QUARTERS.map(q => (
              <button key={q.key} onClick={()=>setActiveQ(q.key)} style={{
                padding:'5px 12px', borderRadius:6, border:'none', cursor:'pointer', fontSize:9, fontWeight:800,
                background: activeQ===q.key ? C.border : 'transparent',
                color: activeQ===q.key ? C.text : C.sub,
                borderBottom: activeQ===q.key ? '2px solid #3b82f6' : '2px solid transparent',
                fontFamily:'inherit',
              }}>{q.label}<br/><span style={{ fontSize:7, fontWeight:400, opacity:0.7 }}>{q.period.slice(0,11)}</span></button>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* ── KPI strip ── */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', background:C.panel, borderBottom:`1px solid ${C.border}` }}>
        {[
          ['Quarter Hedge Recovery', qDef?.rec, '#22c55e'],
          ['Remaining Delay Gap',   qDef?.delay, '#ef4444'],
          ['Quarter Settled Vol',   fmtN(qVol)+' m³', '#3b82f6'],
          ['Active Period',         qDef?.period, col],
        ].map(([lbl,val,clr]) => (
          <div key={lbl} style={{ padding:'12px 20px', borderRight:`1px solid ${C.border}`, textAlign:'center' }}>
            <div style={{ fontSize:8, color:C.sub, textTransform:'uppercase', fontWeight:700, marginBottom:4 }}>{lbl}</div>
            <div style={{ fontSize:18, fontWeight:900, color:clr, fontFamily:'monospace' }}>{val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:'16px 24px', background:C.bg, borderBottom:`1px solid ${C.border}` }}>
        <div style={{ fontSize:9, fontWeight:800, color:C.sub, textTransform:'uppercase', letterSpacing:1, marginBottom:10 }}>
          Category Settlement Summary — Dynamic Remaining %
        </div>
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:10 }}>
            <thead>
              <tr style={{ background:C.tableHead, color:C.text }}>
                {['Category','Total Plan Qty','Cumulative Done','Remaining','Done %','Remaining %'].map((h,i) => (
                  <th key={h} style={{ padding:'7px 14px', textAlign: i>0 ? 'center':'left', fontWeight:800, fontSize:9, letterSpacing:0.5, textTransform:'uppercase', borderRight: i<5?`1px solid ${C.border}`:undefined }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {Object.entries(catTotals).map(([key,{total,done}]) => {
                const rem = Math.max(0, total - done);
                const dp  = total > 0 ? ((done/total)*100).toFixed(1) : '0';
                const rp  = (100 - parseFloat(dp)).toFixed(1);
                const wc  = SETT_CAT_COLORS[key];
                return (
                  <tr key={key} style={{ borderBottom:`1px solid ${C.border}`, background:C.panel }}>
                    <td style={{ padding:'8px 14px', fontWeight:800, borderRight:`1px solid ${C.border}` }}>
                      <span style={{ color:wc, background:wc+'22', padding:'2px 8px', borderRadius:20, border:`1px solid ${wc}44`, fontSize:9 }}>{SETT_CAT_LABELS[key]}</span>
                    </td>
                    <td style={{ padding:'8px 14px', textAlign:'center', fontFamily:'monospace', color:C.text, borderRight:`1px solid ${C.border}` }}>{fmtN(total)}</td>
                    <td style={{ padding:'8px 14px', textAlign:'center', fontFamily:'monospace', color:'#22c55e', fontWeight:700, borderRight:`1px solid ${C.border}` }}>{fmtN(done)}</td>
                    <td style={{ padding:'8px 14px', textAlign:'center', fontFamily:'monospace', color:'#f59e0b', fontWeight:700, borderRight:`1px solid ${C.border}` }}>{fmtN(rem)}</td>
                    <td style={{ padding:'8px 14px', textAlign:'center', fontWeight:900, color:'#3b82f6', borderRight:`1px solid ${C.border}` }}>{dp}%</td>
                    <td style={{ padding:'8px 14px', textAlign:'center', fontWeight:900, color:'#f59e0b' }}>{rp}%</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {/* ── Filters ── */}
      <div style={{ display:'flex', gap:12, padding:'10px 24px', background:C.panel, borderBottom:`1px solid ${C.border}`, alignItems:'center' }}>
        {[['hideFinished','Hide 100% complete', hideFinished, ()=>setHideFinished(v=>!v),'#22c55e'],
          ['hideUnstarted','Hide 0% (not started)', hideUnstarted, ()=>setHideUnstarted(v=>!v),'#ef4444']
        ].map(([id,lbl,val,fn,clr]) => (
          <label key={id} style={{ display:'flex', alignItems:'center', gap:6, cursor:'pointer', padding:'4px 10px', border:`1px solid ${val?clr:C.border}`, borderRadius:6, background:val?clr+'18':C.inputBg }}>
            <input type="checkbox" checked={val} onChange={fn} style={{ cursor:'pointer' }}/>
            <span style={{ fontSize:9, fontWeight:700, color: val?clr:C.sub, textTransform:'uppercase' }}>{lbl}</span>
          </label>
        ))}
      </div>

      {/* ── Zone breakdown ── */}
      <div style={{ padding:'16px 24px', background:C.bg + 'aa' }}>
        {Object.entries(byZone).sort(([a],[b])=>a.localeCompare(b)).map(([zone, zTasks]) => {
          const { qty: zQty, done: zDone } = zoneTotals(zTasks);
          const zPct = zQty > 0 ? ((zDone/zQty)*100).toFixed(1) : '0';
          const visible = zTasks.filter(t => {
            if (!t.active && !t.cumDone) return false;
            if (hideFinished && t.cumPct >= 100) return false;
            if (hideUnstarted && t.overlapDur <= 0) return false;
            return true;
          });
          if (visible.length === 0) return null;
          return (
            <div key={zone} style={{ border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden', marginBottom:16, background:C.card||C.panel, boxShadow:'0 2px 8px rgba(0,0,0,0.06)' }}>
              <div style={{ background:C.panel, padding:'8px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ background:C.tableHead, color:C.text, padding:'2px 8px', borderRadius:4, fontSize:9, fontWeight:800 }}>{zone.replace('Zone ','')}</span>
                  <span style={{ fontWeight:700, fontSize:12, color:col }}>{zone}</span>
                </div>
                <div style={{ display:'flex', gap:20 }}>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize:7, color:C.muted, textTransform:'uppercase', fontWeight:700 }}>Cumulative Done</div>
                    <div style={{ fontSize:13, fontWeight:900, color:'#3b82f6' }}>{zPct}%</div>
                  </div>
                  <div style={{ textAlign:'right', borderLeft:`1px solid ${C.border}`, paddingLeft:16 }}>
                    <div style={{ fontSize:7, color:'#f59e0b', textTransform:'uppercase', fontWeight:700 }}>Remaining</div>
                    <div style={{ fontSize:13, fontWeight:900, color:'#f59e0b' }}>{(100-parseFloat(zPct)).toFixed(1)}%</div>
                  </div>
                </div>
              </div>
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:9 }}>
                  <thead>
                    <tr style={{ background:C.tableHead, color:C.sub, fontSize:8, textTransform:'uppercase' }}>
                      {['Level','Activity','Total Qty','Q Days','Q Alloc','Remaining','Alloc %','Cum %'].map((h,i) => (
                        <th key={h} style={{ padding:'5px 10px', textAlign: i<2 ? 'left':'center', fontWeight:700, borderRight: i<7?`1px solid ${C.border}`:undefined, whiteSpace:'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {visible.map((t,i) => {
                      const isDone = t.cumPct >= 100;
                      const isActive = t.overlapDur > 0;
                      const wc = SETT_CAT_COLORS[t.type] || col;
                      return (
                        <tr key={t.id||i} style={{ background: isDone ? '#22c55e08' : isActive ? C.tableRow1 : C.tableRow0, borderLeft: isDone ? '3px solid #22c55e' : isActive ? '3px solid #3b82f6' : `3px solid ${C.border}`, borderBottom:`1px solid ${C.border}` }}>
                          <td style={{ padding:'6px 10px', fontWeight:700, color:C.sub, borderRight:`1px solid ${C.border}` }}>{t.level}</td>
                          <td style={{ padding:'6px 10px', maxWidth:220, borderRight:`1px solid ${C.border}` }}>
                            <div style={{ fontWeight:700, color:C.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={t.activity}>{t.activity}</div>
                            <span style={{ fontSize:7, color:wc, background:wc+'18', padding:'1px 5px', borderRadius:10, border:`1px solid ${wc}33` }}>{SETT_CAT_LABELS[t.type]}</span>
                          </td>
                          <td style={{ padding:'6px 10px', textAlign:'center', fontFamily:'monospace', color:C.text, borderRight:`1px solid ${C.border}` }}>{fmtN(t.qty)}</td>
                          <td style={{ padding:'6px 10px', textAlign:'center', color:'#3b82f6', fontWeight:700, fontFamily:'monospace', borderRight:`1px solid ${C.border}` }}>{t.overlapDur}d</td>
                          <td style={{ padding:'6px 10px', textAlign:'center', color:'#22c55e', fontWeight:800, fontFamily:'monospace', borderRight:`1px solid ${C.border}` }}>{fmtN(t.qQty)}</td>
                          <td style={{ padding:'6px 10px', textAlign:'center', color:'#f59e0b', fontFamily:'monospace', borderRight:`1px solid ${C.border}` }}>{fmtN(t.qty - t.cumDone)}</td>
                          <td style={{ padding:'6px 10px', textAlign:'center', fontWeight:800, color:'#3b82f6', borderRight:`1px solid ${C.border}` }}>{t.allocPct}%</td>
                          <td style={{ padding:'6px 10px', textAlign:'center', fontWeight:900, color: t.cumPct>=100?'#22c55e':t.cumPct>0?'#f59e0b':'#ef4444' }}>
                            {t.cumPct}%
                            <div style={{ width:'100%', height:3, background:C.inputBg, borderRadius:99, marginTop:2 }}>
      ...
      {/* ── Footer ── */}
      <div style={{ padding:'14px 24px', background:C.tableHead, color:C.text, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ display:'flex', gap:40 }}>
          <div>
            <div style={{ fontSize:8, color:'#475569', textTransform:'uppercase', fontWeight:700, marginBottom:4 }}>Overall Project Progress</div>
            <div style={{ fontSize:20, fontWeight:900, color:'#60a5fa', fontFamily:'monospace' }}>{projPct}%</div>
          </div>
          <div>
            <div style={{ fontSize:8, color:'#f59e0b', textTransform:'uppercase', fontWeight:700, marginBottom:4 }}>Total Remaining Volume</div>
            <div style={{ fontSize:20, fontWeight:900, color:'#f59e0b', fontFamily:'monospace' }}>{fmtN(projTotal - projDone)}</div>
          </div>
        </div>
        <div style={{ fontSize:10, color:'#3b82f6', fontWeight:700, fontStyle:'italic', textTransform:'uppercase' }}>
          Settlement Core V10 · Time-Ratio Model
        </div>
      </div>
    </div>
  );
