...
function LogisticsView({ C, lightMode }) {
  ...
  useEffect(() => {
    if (!chartJsReady2 || !ktcChartData || !ktcChartRef.current) return;
    ...
    const maxVol  = Math.max(...ktcChartData.vols, 1);
    const peak    = ktcChartData.vols.indexOf(Math.max(...ktcChartData.vols));
    ktcChartInst.current = new window.Chart(ktcChartRef.current, {
      type: 'bar',
      data: {
        labels: ktcChartData.labels,
        datasets: [
          {
            label: 'Volume (m³)',
            data: ktcChartData.vols,
            backgroundColor: ktcChartData.vols.map((_,i) => i===peak ? '#fb923c' : 'rgba(251,146,60,0.35)'),
            borderColor:     ktcChartData.vols.map((_,i) => i===peak ? '#fb923c' : 'rgba(251,146,60,0.6)'),
            borderWidth: 1, borderRadius: 4, borderSkipped: false,
            yAxisID: 'yVol',
          },
          {
            label: 'Trips',
            data: ktcChartData.trips,
            type: 'line',
            borderColor: '#f59e0b',
            backgroundColor: 'transparent',
            borderWidth: 2,
            pointRadius: ktcChartData.labels.length > 20 ? 2 : 4,
      ...
      options: {
        ...
        scales: {
          x: {
            grid: { display: false },
            ticks: { color: tickCol, font: { size: 10 }, maxRotation: 45, autoSkip: ktcChartData.labels.length > 20 },
          },
          yVol: {
            position: 'left',
            grid: { color: gridCol },
  ...
  }, [ktcChartData, chartJsReady2, lightMode]);
